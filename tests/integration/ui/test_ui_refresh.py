...
@pytest.fixture
def study():
    """Return a real empty Study for navigation refresh wiring checks."""
    return Study()


def test_ui_refresh_on_tab_switch(qtbot, study):
    ...
    with (
        ...
        cast(Any, MockDatasetPanel.return_value).update_panel = MagicMock()
        cast(Any, MockPreprocessPanel.return_value).update_panel = MagicMock()
        cast(Any, MockTrainingPanel.return_value).update_panel = MagicMock()
        cast(Any, MockVisPanel.return_value).update_panel = MagicMock()
        cast(Any, MockEvalPanel.return_value).update_panel = MagicMock()
        ...
        vis_panel = cast(Any, window.visualization_panel)
        eval_panel = cast(Any, window.evaluation_panel)
