...
@pytest.mark.parametrize(
    ("has_model", "embedding_ready", "expected_error"),
    (
        (True, False, None),
        (False, True, "Pinned local model cache is missing"),
    ),
)
def test_launch_requires_the_local_model_but_not_optional_rag_embedding(
    ...
    """Exercise launch preflight through its real checkout guard, not source text."""
    source_root = Path(manual.__file__).resolve().parents[2]
    sha = "a" * 40
    run = tmp_path / "manual-run"
    cache = tmp_path / "shared-cache"
    executed: list[str] = []
    original_run_path = manual.runpy.run_path
    from XBrainLab.llm.core import config as llm_config
    from XBrainLab.llm.rag import config as rag_config

    class FakeLLMConfig:
        def has_local_model_cache(self) -> bool:
            return has_model

    class FakeRAGConfig:
        @staticmethod
        def embedding_cache_ready() -> bool:
            return embedding_ready

    def run_path(path: str, *, run_name: str | None = None):
        if path.endswith("active_checkout.py"):
            return original_run_path(path, run_name=run_name)
        executed.append(path)
        return {}

    monkeypatch.setattr(llm_config, "LLMConfig", FakeLLMConfig)
    monkeypatch.setattr(rag_config, "RAGConfig", FakeRAGConfig)
    fake_os = SimpleNamespace(
        name="nt",
        environ=dict(manual.os.environ),
        chdir=lambda _path: None,
    )
    monkeypatch.setattr(manual, "os", fake_os)
    monkeypatch.setattr(manual, "verify_source", lambda *_args: sha)
    monkeypatch.setattr(manual, "verify_environment", lambda *_args: None)
    monkeypatch.setattr(manual, "manual_lock", lambda _source: nullcontext())
    monkeypatch.setattr(manual, "_run_root", lambda *_args: run)
    monkeypatch.setattr(manual.runpy, "run_path", run_path)
    monkeypatch.setattr(sys, "argv", list(sys.argv))

    if expected_error is not None:
        with pytest.raises(RuntimeError, match=r"model.*cache.*missing"):
            manual.launch(source_root, sha, cache)
        assert executed == []
    else:
        manual.launch(source_root, sha, cache)
        assert executed == [str(source_root / "run.py")]
        assert fake_os.environ["HF_HUB_OFFLINE"] == "1"
        assert fake_os.environ["TRANSFORMERS_OFFLINE"] == "1"
