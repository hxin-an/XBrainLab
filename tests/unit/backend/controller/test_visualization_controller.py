...
def test_get_averaged_record(controller):
    ...
    with patch('XBrainLab.backend.controller.visualization_controller.EvalRecord') as MockRecord:
        controller.get_averaged_record(holder)

        args, kwargs = MockRecord.call_args
        # Args order: label, output, gradient, gradient_input, smoothgrad, smoothgrad_sq, vargrad
        # Check gradient
