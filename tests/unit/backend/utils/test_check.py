...
from XBrainLab.backend.utils import check
...
def test_validate_type():
    ...
    with pytest.raises(
        TypeError,
        match=r"test must be an instance of builtins.float, got <class 'int'> instead.",
    ):
        check.validate_type(1, float, "test")
    with pytest.raises(
        TypeError,
        match=r"test must be an instance of builtins.float or builtins.int, "
...
def test_validate_list_type():
    ...
    with pytest.raises(
        TypeError,
        match=r"Items of test must be an instance of builtins.float, "
