...
def test_parse_filename(raw):
    ...
    # Invalid regex should be handled gracefully without crashing or printing
    # stack trace
    raw.parse_filename("sub-(?P<subject>[^_]*_ses-(?P<session>[^_]*)_.*")
    assert raw.get_subject_name() == "01"
    assert raw.get_session_name() == "01"
...
def mne_raw_stim():
    ...
    info = mne.create_info(
        ch_names=["O1", "O2", "STI"], sfreq=fs, ch_types=["eeg", "eeg", "stim"]
    )
...
def mne_raw_annot():
    ...
    mne_raw.set_annotations(
        mne.Annotations(
            onset=[0.1, 0.3, 0.5], duration=[0.2, 0.2, 0.2], description=["a", "b", "c"]
        )
    )
    return mne_raw
