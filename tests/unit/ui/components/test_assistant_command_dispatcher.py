...
class _QueuedController(QObject):
    def __init__(self) -> None:
        super().__init__()
        self.calls: list[str] = []
        self.initialization_thread: QThread | None = None
        self.navigation_resolutions = []

    def initialize(self, _launch_spec: AssistantRuntimeLaunchSpec) -> None:
        self.initialization_thread = QThread.currentThread()
    ...
    def on_panel_navigation_resolved(self, request, success) -> None:
        self.calls.append("resolve_panel_navigation")
        self.navigation_resolutions.append((request, success, QThread.currentThread()))
...
def _dispatch_all(dispatcher: AssistantCommandDispatcher) -> dict[str, bool]:
    ...
    return {
        ...
        "resolve_panel_navigation": dispatcher.resolve_panel_navigation(
...
def test_queued_dispatch_delivers_every_supported_command(qtbot: Any) -> None:
    controller = _QueuedController()
    dispatcher = AssistantCommandDispatcher()
    dispatcher.bind(controller)
    try:
        deliveries = _dispatch_all(dispatcher)
        assert all(deliveries.values())
        qtbot.waitUntil(lambda: len(controller.calls) == len(deliveries))
        assert controller.calls == list(deliveries)
    finally:
...
def test_dispatch_operations_reject_after_closing_starts(qtbot: Any) -> None:
    controller = _RetryingAsyncClosingQueuedController()
    dispatcher = AssistantCommandDispatcher()
    dispatcher.bind(controller)
    cleanup_results: list[bool] = []
    dispatcher.cleanup_finished.connect(lambda ok, _message: cleanup_results.append(ok))
    try:
        assert dispatcher.close() is False
        qtbot.waitUntil(lambda: cleanup_results == [False])
...
def test_navigation_resolution_preserves_outcome_on_owned_command_thread(
    qtbot, success
):
    controller = _QueuedController()
    dispatcher = AssistantCommandDispatcher()
    ...
    try:
        assert dispatcher.resolve_panel_navigation(request, success=success)
        qtbot.waitUntil(lambda: bool(controller.navigation_resolutions), timeout=2_000)
        assert controller.navigation_resolutions == [
            (request, success, dispatcher.command_thread)
        ]
        assert controller.navigation_resolutions[0][2] is not dispatcher.thread()
    finally:
        _finish_queued_close(qtbot, dispatcher)
...
def test_qobject_without_generation_worker_uses_owned_command_thread(qtbot) -> None:
    controller = _QueuedController()
    dispatcher = AssistantCommandDispatcher()
    dispatcher.bind(controller)
    try:
        assert not hasattr(controller, "worker_thread")
        assert dispatcher.initialize(_launch_spec())
        qtbot.waitUntil(lambda: controller.initialization_thread is not None)
        assert controller.initialization_thread is dispatcher.command_thread
        assert controller.initialization_thread is not QThread.currentThread()
    finally:
        _finish_queued_close(qtbot, dispatcher)


def test_bind_rejects_non_qt_transport_without_taking_ownership() -> None:
    from types import SimpleNamespace

    controller = SimpleNamespace()
    for name in AssistantCommandDispatcher._REQUIRED_CONTROLLER_METHODS:
        setattr(controller, name, lambda *args: None)
    dispatcher = AssistantCommandDispatcher()

    with pytest.raises(TypeError, match="QObject controller"):
        dispatcher.bind(controller)
