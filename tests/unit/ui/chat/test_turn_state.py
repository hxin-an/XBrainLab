...
def test_rejected_provisional_submission_preserves_exact_stopping_lease() -> None:
    ...
    assert state.complete_admission(first, stopped) is True
...
def test_only_successful_admission_may_supersede_a_stopping_generation() -> None:
    ...
    assert state.complete_admission(replacement, admitted) is True
...
def test_missing_correlation_is_never_a_wildcard_and_double_stop_is_idempotent() -> (
    ...
    assert state.complete_admission(submission, active) is True
...
def test_reused_runtime_turn_id_is_scoped_by_ui_generation_not_poisoned_globally() -> (
    ...
    assert state.complete_admission(first_submission, first) is True
    ...
    assert state.complete_admission(second_submission, reused) is True
...
def test_superseded_completion_cannot_clear_the_newer_reservation() -> None:
    state = AssistantUiTurnStateMachine()
    older = state.begin_submission()

    newer = state.begin_submission()

    assert state.complete_admission(older, _correlation(older.generation, 61)) is False
...
def test_turn_presentation_state_survives_terminal_acceptance_until_rendering_finishes() -> (
    ...
    assert state.complete_admission(submission, correlation) is True
...
def test_stale_rejection_cannot_clear_the_newer_reservation() -> None:
    state = AssistantUiTurnStateMachine()
    older = state.begin_submission()
    newer = state.begin_submission()
