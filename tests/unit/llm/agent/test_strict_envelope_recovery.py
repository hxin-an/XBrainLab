...
def test_recovery_taxonomy_preserves_each_structured_response_branch() -> None:
    policy = StrictEnvelopeRecoveryPolicy(max_recovery_attempts=2)
    examples = {
        "blocked": (
            '{"tool_name":"respond_to_user","parameters":{'
            '"decision":"blocked","message":"Load data before training."}}'
        ),
        "missing_input": (
            '{"tool_name":"respond_to_user","parameters":{'
        ...
        "answer": (
            '{"tool_name":"respond_to_user","parameters":{'
            '"decision":"answer","message":"An epoch is a time window."}}'
        ),
    }
    expected = {
        "blocked": StrictEnvelopeRecoveryTaxonomy.FIRST_ATTEMPT_BLOCKED,
        "missing_input": (StrictEnvelopeRecoveryTaxonomy.FIRST_ATTEMPT_MISSING_INPUT),
...
def test_second_attempt_answer_maps_to_recovered_answer_taxonomy() -> None:
    policy = StrictEnvelopeRecoveryPolicy(max_recovery_attempts=2)
    envelope = CommandParser.parse_product(
        '{"tool_name":"respond_to_user","parameters":{'
        '"decision":"answer","message":"An epoch is a time window."}}'
    ...
    assert decision.action is StrictEnvelopeRecoveryAction.ACCEPT_NO_TOOL
    assert decision.taxonomy is StrictEnvelopeRecoveryTaxonomy.RECOVERED_ANSWER
