...
# ruff: noqa: B007, B008, B905, E501, N801, PGH003, S101, UP035
...
# Names and order match the channel-embedding row order used during pre-training.
...
def _resolve_channel_embedding_config(
    ...
    if missing:
        raise ValueError(
            ...
            "Use channel_embedding='scratch' and load a compatible state dict "
            "with strict=False if channel-embedding weights are intentionally omitted."
        )
...
class SignalJEPA(_BaseSignalJEPA):
    ...
    * :class:`SignalJEPA_PreLocal`
    ...
    The classification architectures are instantiated from local constructor inputs.
