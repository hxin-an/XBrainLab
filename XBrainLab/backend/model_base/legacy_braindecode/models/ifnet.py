...
class IFNet(EEGModuleMixin, nn.Module):
    ...
    def __init__(
        ...
        self.bands = [(4.0, 16.0), (16, 40)] if bands is None else bands
...
class _SpatioTemporalFeatureBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.n_bands != len(self.kernel_sizes):
            warn(
                f"Got {self.n_bands} bands, different from "
                f"{len(self.kernel_sizes)} amount of "
            ...
            if self.n_bands > len(self.kernel_sizes):
                ...
                warn(
                    f"Reducing number of bands to {len(self.kernel_sizes)} to "
                    "match the number of kernels.",
                    UserWarning,
                )
            elif self.n_bands < len(self.kernel_sizes):
                self.kernel_sizes = self.kernel_sizes[: self.n_bands]
                warn(
                    f"Reducing number of kernels to {self.n_bands} to match "
                    "the number of bands.",
                    UserWarning,
                )
