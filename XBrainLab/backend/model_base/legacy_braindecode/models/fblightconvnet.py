...
class FBLightConvNet(EEGModuleMixin, nn.Module):
    ...
    n_bands : int or None or list of tuple of int, default=8
        Number of frequency bands or a list of frequency band tuples. If a list
        of tuples is provided, each tuple defines the lower and upper bounds of
        a frequency band.
    ...
    def __init__(
        ...
        verbose: bool = False,
        filter_parameters: dict | None = None,
    ):
        super().__init__(
            n_chans=n_chans,
            n_outputs=n_outputs,
...
class _LightweightConv1d(nn.Module):
    ...
    weight_softmax : bool, optional
        If `True`, normalizes the convolution weights with softmax before applying
        the convolution.
        Default is `False`.
    bias : bool, optional
    ...
    def forward(self, x):
        ...
        batch_size, channels, n_times = x.size()
        ...
        x = x.view(-1, heads, n_times)
        output = functional.conv1d(x, weight, padding=self.padding, groups=self.heads)
        # 4, 8, 1
