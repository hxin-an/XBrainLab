...
class STEEGFormer(EEGModuleMixin, nn.Module):
    ...
    def _chan_pos_idx_from_chs_info(self) -> torch.Tensor:
        """Use identity mapping only when no montage metadata is supplied.

        The legacy provider is deliberately offline and does not fetch the
        upstream channel vocabulary. A caller with montage metadata must pass
        the reviewed ``chan_pos_idx`` mapping explicitly.
        """
        try:
        ...
        raise ValueError(
            "STEEGFormer legacy recovery does not download channel metadata; "
            "pass chan_pos_idx explicitly when chs_info is present."
        )
