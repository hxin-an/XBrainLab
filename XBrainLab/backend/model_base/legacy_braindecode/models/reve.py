...
"""
# ruff: noqa: E501, N812, RUF002, UP007, UP045
...
class REVE(EEGModuleMixin, nn.Module):
    ...
    **R**\ epresentation for **E**\ EG with **V**\ ersatile **E**\ mbeddings (REVE) from El Ouahidi et al. (2025) [reve]_.
    ...
    Notes
    -----
    The local legacy implementation never retrieves an electrode-position bank.
    Callers must pass explicit channel positions to :meth:`forward`; channel-name
    lookup fails closed when no local positions were supplied.
    """
    ...
    def __init__(
        ...
        self.fourier4d = _FourierPositionEmbedding4D(self.embed_dim, freqs=self.freqs)
    ...
    def get_positions(self, channel_names: list[str]) -> torch.Tensor:
        """Fetch channel positions from the local position bank.
    ...
    def forward(
        ...
        pos = _FourierPositionEmbedding4D.add_time_patch(pos, n_patches)
...
class _FourierPositionEmbedding4D(nn.Module):
    """
...
class RevePositionBank(torch.nn.Module):
    """Fail-closed local position lookup used by the legacy REVE source.

    XBrainLab does not redistribute or retrieve the upstream electrode-position
    dataset. The normal legacy execution path supplies positions directly to
    :meth:`REVE.forward`; channel-name lookup remains unavailable.
    """

    def __init__(self) -> None:
        super().__init__()
        self.register_buffer(
            "embedding", torch.empty((0, 3), dtype=torch.float32), persistent=False
        )

    def forward(self, channel_names: list[str]) -> torch.Tensor:
        del channel_names
        raise RuntimeError(
            "REVE legacy execution requires explicit channel positions; pass pos to "
            "REVE.forward. The local provider never retrieves an electrode-position bank."
        )
