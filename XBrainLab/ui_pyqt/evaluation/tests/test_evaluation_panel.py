...
def test_evaluation_panel_refresh_no_data(mock_main_window, qtbot):
    ...
    # Should show warning but not crash
    with patch('XBrainLab.ui_pyqt.evaluation.panel.QMessageBox.warning') as mock_warn:
        panel.refresh_data()
...
def test_evaluation_panel_refresh_with_data(mock_main_window, qtbot):
    ...
    with patch('XBrainLab.ui_pyqt.evaluation.panel.EvaluationTableWidget', side_effect=DummyWidget) as MockTable, \
         patch('XBrainLab.ui_pyqt.evaluation.panel.ConfusionMatrixWidget', side_effect=DummyWidget) as MockMatrix, \
         patch('XBrainLab.ui_pyqt.evaluation.panel.QMessageBox.information') as mock_info:
...
def test_evaluation_panel_export_placeholders(mock_main_window, qtbot):
    ...
    with patch('XBrainLab.ui_pyqt.evaluation.panel.QMessageBox.information') as mock_info:
