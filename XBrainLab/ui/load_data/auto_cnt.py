...
class AutoCnt:
    ...
    def load_data(self):
        ...
        for i, data_path in enumerate(self.data_paths):
            ...
            if data_path.endswith('.cnt'):
                mne_data = mne.io.read_raw_cnt(data_path, preload=True)
                self.script_history.add_cmd(
                    "data = mne.io.read_raw_cnt(filepath, preload=True)"
                )
            else:
                raise ValueError(f"Unsupported file type for {data_path}. Only CNT (.cnt) files are supported.")
