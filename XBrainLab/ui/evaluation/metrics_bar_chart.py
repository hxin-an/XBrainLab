...
class MetricsBarChartWidget(QWidget):
    ...
    def update_plot(self, metrics):
        ...
        try:
            ...
            # Extract data
            classes = [k for k in metrics.keys() if isinstance(k, int)]
            classes.sort()

            if not classes:
                self.ax.text(0.5, 0.5, "No Class Data", color='#666666', ha='center', va='center')
                self.ax.axis('off')
                self.canvas.draw()
                return
            ...
            # Plot Bars
            rects1 = self.ax.bar(x - width, precision, width, label='Precision', color='#4CAF50')
            rects2 = self.ax.bar(x, recall, width, label='Recall', color='#2196F3')
            rects3 = self.ax.bar(x + width, f1, width, label='F1-Score', color='#FFC107')
