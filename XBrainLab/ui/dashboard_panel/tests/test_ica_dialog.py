...
from XBrainLab.ui.dashboard_panel.preprocess import ICADialog
...
def test_ica_dialog_init(mock_data, qtbot):
    ...
    with patch('XBrainLab.backend.preprocessor.ICA'):
...
def test_ica_dialog_accept(mock_data, qtbot):
    ...
    with patch('XBrainLab.backend.preprocessor.ICA') as MockICA:
        dialog = ICADialog(None, mock_data)
        qtbot.addWidget(dialog)
        ...
        with patch('PyQt6.QtWidgets.QProgressDialog') as MockProgress:
            ...
            # Mock ICAWorker
            with patch('XBrainLab.ui.dashboard_panel.preprocess.ICAWorker') as MockWorker:
