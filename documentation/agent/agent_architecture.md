...
1. **System Prompt** — 根據 Pipeline Stage 選擇專用的 system_prompt
2. **Tool Definitions** — 根據 `STAGE_CONFIG` 動態插入當前階段可用工具 Schema
3. **RAG Context** — 從 Qdrant 檢索的 Few-Shot 相似案例
4. **Memory** — 對話歷史（Sliding Window）

### 2.3 Pipeline State Machine (`pipeline_state.py`)
**職責**: 定義 EEG 處理流水線的狀態及每階段允許的工具與 system prompt。

**階段**:
- `EMPTY` → `DATA_LOADED` → `PREPROCESSED` → `EPOCHED` → `DATASET_READY` → `TRAINING`

**核心配置**: `STAGE_CONFIG` — 每階段定義 `tools` (允許的工具名列表) 與 `system_prompt` (階段專用系統提示)。

### 2.4 Confidence Estimator (`confidence.py`)
**職責**: 基於關鍵字匹配估算 LLM 回應的信心度，用於 VerificationLayer 的信心度閾值檢查。

### 2.5 Verification Layer (`verifier.py`)
...
> **注意**: 信心度檢查已透過 `confidence.py` 模組啟用。

### 2.6 Agent Worker (`worker.py`)
...
2. `ChatPanel` → `AgentManager` → `agent_controller.handle_user_input()`
...
```
XBrainLab/llm/
├── pipeline_state.py         # Pipeline Stage 狀態機 (STAGE_CONFIG)
├── agent/                    # 控制層
...
│   ├── confidence.py         # 信心度估算 (關鍵字匹配)
│   ├── conversation.py       # 對話歷史管理 (ConversationHistory)
