...
*   **PipelineStage**: 流水線階段枚舉。定義 EEG 處理的六個有序階段 (`EMPTY` → `DATA_LOADED` → `PREPROCESSED` → `EPOCHED` → `DATASET_READY` → `TRAINING`)，用於狀態機驅動的工具過濾與 system prompt 選擇。
*   **ConversationHistory**: 對話歷史管理器。封裝 Sliding Window 策略的對話記憶體，管理 `MAX_HISTORY` 限制與訊息增刪。
*   **estimate_confidence**: 信心度估算函式。位於 `confidence.py`，基於關鍵字匹配分析 LLM 回應文本的確定性，回傳 0.0–1.0 的信心分數，供 VerificationLayer 決定是否放行工具呼叫。
