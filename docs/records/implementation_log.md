...
- `scripts/launchers/xbrainlab_wsl_launcher.ps1` 新增
  ...
  - `cd` active repo。
  - 最新版本在 startup smoke 內強制 `XBRAINLAB_STARTUP_DIAGNOSTICS=1`。
...
- 新增 `scripts/dev/capture_windows_launcher_walkthrough.py`：
  ...
  - 最新版本會檢查 startup geometry diagnostics：screen count、screen detail、splash geometry、
    main-window geometry。
  - 保存 JSON / Markdown artifact。
- 新增 `tests/unit/scripts/test_capture_windows_launcher_walkthrough.py`，覆蓋 log path parsing 和
  artifact claim boundary rendering；最新版本也覆蓋 geometry diagnostics parser。
...
- artifact summary:
  ...
  - startup geometry diagnostics checks passed
...
- focused static checks:
  ...
  - pass
  - `poetry run ruff format --check scripts/dev/capture_windows_launcher_walkthrough.py tests/unit/scripts/test_capture_windows_launcher_walkthrough.py`
  - pass
...
使用者要求因用量將近用完，先建立交接記錄，等用量刷新後再繼續 product completion work。

### 變更

- `artifacts/goal/handoff-2026-05-04-usage-refresh.md`
  - 以 `3ffa73d mcp: verify inspector cli config` 作為最新接續點。
  - 補齊本輪已完成 slices：MCP client release config、generic label carrier manual target
    mapping、label target selector UX、MCP Inspector CLI smoke。
  - 明確標出 expected dirty files 只剩 protected `.vscode/settings.json` 和 root
    `settings.json`。
  - 記錄下一步建議 slice：Data Interpretation format capability matrix。
...
- 這是交接文件刷新，不是 product completion。
- Goal 不能標 complete。
...
先前 Data Interpretation 已在 scan / preview 顯示 format capability boundaries，但 evidence
散在 backend test、UI replay 和文字紀錄中。下一個 product slice 需要一份可重跑 artifact，
明確列出目前支援、需確認、context-only、blocked 的格式邊界。

### 變更

- `scripts/dev/report_data_interpretation_format_matrix.py`
  - 新增 synthetic scan fixtures 和 live ApplicationService command runner。
  - 每個 case 走 `ScanSourceCommand`、`PreviewInterpretationCommand`、
    `ValidateInterpretationCommand`。
  - 輸出 rows、case summaries、status coverage、validation decision coverage 和 claim boundary。
  - CLI `--format json` 會保持 stdout 純 JSON；ApplicationService info logs 在 reporter 執行時
    會被壓到 warning 以上。
- `artifacts/data_interpretation/format-capability-matrix.json`
  - checked-in machine-readable matrix。
...
- `tests/unit/scripts/test_report_data_interpretation_format_matrix.py`
  - 覆蓋 matrix coverage、Markdown claim boundary，以及 CLI JSON machine-readability。
...
- BIDS `events.tsv`。
- TXT labels。
- XDF / LSL stream export blocked boundary。

### 驗證

- TDD failure:
  - `poetry run pytest --capture=sys tests/unit/scripts/test_report_data_interpretation_format_matrix.py -q`
  - failed with missing `scripts.dev.report_data_interpretation_format_matrix` module。
- CLI JSON failure:
  - focused CLI test failed because `Study initialized` logs preceded JSON stdout。
- Final:
  - `poetry run pytest --capture=sys tests/unit/scripts/test_report_data_interpretation_format_matrix.py -q`
  - `3 passed`
  ...
  - wrote `artifacts/data_interpretation/format-capability-matrix.json` / `.md`。
  - `poetry run mkdocs build --strict` -> pass with existing MkDocs Material warning。
  - `poetry run python tests/architecture_compliance.py` -> Architecture compliant。
  - `git diff --check` -> pass。
...
- 這支撐 generated Data Interpretation format capability-boundary matrix。
- 這不支撐 XDF / LSL stream parser、raw-event-anchor-specific MAT/GDF alignment、full real-data
  manual certification、Inspector GUI、Windows launcher click-through 或完整 product completion。

## 2026-05-04 Data Interpretation reviewed MAT sample-anchor apply

### 背景

Data Interpretation 已能把 MAT / TXT trial-order labels 走 legacy sequence import，也能把
CSV / TSV / BIDS timestamp labels 走 timestamp batch import。但 reviewed MAT `classlabel` +
`cue_onset` 這類 sample-index anchor 還只能停在 recipe review；`apply_interpretation` 會 skipped。

### 變更

- `XBrainLab/backend/load_data/label_loader.py`
  - `_load_mat()` 新增 reviewed `anchor` 參數。
  - MAT anchor 存在時，loader 會解析 label variable 和 anchor variable，檢查長度一致、anchor
    為 finite integer sample indexes，並輸出 MNE-style event array。
- `XBrainLab/backend/application/service.py`
  - `_apply_interpretation_label_carriers()` 新增 `anchored` mode。
  - `_is_auto_applicable_anchored_label_plan()` 要求 MAT carrier、selected label、selected anchor、
    `time_model=sample_index`、`granularity=trial` 和 confirmed class map。
  - anchored mode 走 `apply_labels_batch`，`_load_reviewed_label_map()` 會把 anchor 傳給
    `load_label_file()`。
  - recipe trace 使用 `label_import:anchored:<n>`。
- `tests/unit/backend/load_data/test_label_loader_coverage.py`
  - 新增 MAT label + sample anchor event-array test。
- `tests/unit/backend/application/test_application_service.py`
  - 新增 reviewed MAT sample anchor apply test。

### 驗證

- TDD failure:
  - focused label loader test failed because loader returned `array([1, 2, 1])` instead of
    `[[100, 0, 1], [250, 0, 2], [400, 0, 1]]`。
  - focused ApplicationService test failed because `label_apply.status` was `skipped`。
- Final:
...
Role review 已進入 Data Interpretation recipe UI，但 label carrier table 仍靠 editable text cells。
使用者需要手打 `sample_index` / `trial` 這類 backend value，產品感不足，也容易輸入錯誤。

### 變更

- `XBrainLab/ui/dialogs/dataset/data_interpretation_preview_dialog.py`
  - 新增 `_label_choice_widgets`，用 `(item id, column)` 追蹤 label carrier selector widgets。
  - label field、anchor、time model、granularity 和 role 欄位改成 `QComboBox`。
  - controlled values 顯示人話 label，例如 `Seconds`、`Trial`、`Class cue labels`。
  - `_label_carrier_item_text()` 會優先讀取 combo `itemData`，因此 result 仍是 backend recipe
    value。
- `tests/unit/ui/dialogs/dataset/test_data_interpretation_preview_dialog.py`
  - 新增 selector-focused test。
  - 更新既有 label carrier review test，改用 selector 模擬使用者操作。
- `scripts/dev/capture_data_interpretation_replay.py`
  - replay 新增 `set_tree_cell()`，若欄位有 combo widget 會透過 combo 設值。
  - replay visible row 現在顯示 `Seconds` / `Trial` / `Class cue labels`。
- `artifacts/ui/data-interpretation-preview.png`
- `artifacts/ui/data-interpretation-applied.png`
- `artifacts/ui/data-interpretation-replay.json`
...
- TDD failure:
  - `test_data_interpretation_preview_dialog_uses_label_carrier_selectors` failed because selector
...
- Final:
  ...
  - `1 passed`
  - `xvfb-run -a poetry run python scripts/dev/capture_data_interpretation_replay.py` -> exit `0`
  - replay JSON visible row ends with `Seconds`, `Trial`, `Class cue labels`
  - replay JSON choices still store `time_model=seconds`, `granularity=trial`,
...
- 這支撐 Data Interpretation label carrier review 的 selector UX。
- 這不支撐完整 embedded post-load label import editor、raw trigger selector、all-format
  real-data certification、MCP Inspector GUI、Windows launcher click-through 或完整 product
  completion。
...
`Add Labels to Loaded Data` 已被降為 compatibility path，但 sidebar button 和 action guard 仍太像
舊 `attach_labels` 心智模型：empty state 仍可能問 `Apply to ALL?`，且 UI action 沒有先讀 backend
`ImportLabelsCommand` capability。這會讓使用者在沒有資料或 downstream locked state 下看到錯誤入口。

### 變更

- `XBrainLab/ui/panels/dataset/sidebar.py`
  - empty state disable `Add Labels to Loaded Data`。
  - locked state disable `Add Labels to Loaded Data`。
  - loaded/unlocked state tooltip 明確說明會 update current recipe trace。
- `XBrainLab/ui/panels/dataset/actions.py`
  - `import_label()` 先讀 `get_command_capability(..., CommandName.IMPORT_LABELS)`。
  - capability disabled 時顯示 `Label Import Blocked`，使用 backend reason。
  - `_get_target_files_for_import()` 在 table row count 為 `0` 時直接 warning，不再詢問 apply all。
- `scripts/dev/capture_data_interpretation_replay.py`
  - replay JSON 新增 empty-state 和 applied-state Add Labels button state。
- tests:
  - sidebar locked / empty-state tests。
  - action no-data / capability-block tests。

### 驗證

- TDD failures:
  - sidebar button remained enabled in locked / empty states。
  - action did not warn on empty table rows。
  - action ignored backend capability block。
- Final:
  ...
  - `54 passed`
  - `xvfb-run -a poetry run python scripts/dev/capture_data_interpretation_replay.py` -> exit `0`
  - replay JSON empty state button enabled `False` with tooltip `Interpret a data source before adding labels.`
  - replay JSON applied state button enabled `True` with tooltip `Add labels to loaded data and update the current recipe trace.`
  - targeted `ruff check` / `ruff format --check` clean
  - targeted `basedpyright` clean

### 不能宣稱完成

- 這支撐 post-load label compatibility path 的 state/capability guard。
- 這不支撐完整 embedded Data Interpretation label editor、raw trigger selector、all-format
  real-data certification、MCP Inspector GUI、Windows launcher click-through 或完整 product
  completion。
