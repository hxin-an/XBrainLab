...
| Data Interpretation | 已有 scan -> preview -> validate -> confirm/apply -> save recipe 的第一版 UI / backend 主線；metadata、class map、event role、label carrier plan 和 format boundaries 可預覽與保存。 |
...
| UI Product Experience | ChatPanel 已從 debug dock 改成產品面板；有 true local-model 回覆、tool command、多輪、Data Interpretation chain、pipeline chain、training boundary / completion walkthrough artifacts。 |
| Agent / Local LLM | local primary / fallback 非中國模型已跑 `100` thesis-candidate tool-call cases，各 `3` 次，artifact 顯示 `100 / 100`。 |
| Automation / MCP | real stdio MCP server 已存在，tool schema 來自同一套 ApplicationService automation truth；stdlib-only client walkthrough 和 official Inspector CLI `tools/list` release-config baseline 已保存。 |
...
- MCP 已有 real stdio server、client config、CLI walkthrough；MCP Inspector GUI click-through
  仍在施工中，不能宣稱 external GUI client 已完整驗收。
...
- Tool-call benchmark 已達 thesis-candidate case 數與 primary / fallback x3 重跑；這支撐
  tool-call benchmark claim，不等於整個桌面產品或論文實驗最終 closure。

### 下一手重點

1. 完成 MCP Inspector GUI click-through artifact，並保證腳本自動清理 Windows node / Chrome
   process。
2. 收斂 Data Interpretation label editor / raw event anchor / XDF-LSL boundary，避免 wizard
   與 post-load compatibility path 形成雙主線。
3. 做 Windows Desktop 真人 click-through 或更接近真人行為的 observable walkthrough。
4. 補 interactive desktop 3D / PyVista render 驗證，或把 release boundary 寫得更明確。
5. 將 roadmap、current、validation 和 records 保持同步，但避免把 worklog 細節複製進
   implementation log。
