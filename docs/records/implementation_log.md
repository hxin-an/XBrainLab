...
- 最新使用者要求的 UI-observable human-like walkthrough 尚未形成單一完整 artifact；現有
  UI artifacts 是多條 sliced walkthrough，不能替代真人 Windows / 雙螢幕 / DPI desktop
  acceptance。
...
2. 做最新要求的 automated human-like walkthrough artifact，涵蓋資料入口、ChatPanel、reset /
   recovery、visible text、button state、backend state snapshot 和 transcript。
