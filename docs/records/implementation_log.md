...
- Backend-ready long-running `train` over HTTP returns `long_running_job_required` instead of
...
- This is not an HTTP job API. Long-running progress / cancel / recovery, remote authorization
  certification, Windows human launcher acceptance, and full MCP client certification remain open.

### 下一手重點

- Design the HTTP job boundary before allowing long-running training / evaluation / visualization
  jobs through MCP HTTP.
...
5. 硬化 MCP HTTP job progress / cancel / recovery boundary，不把 Inspector GUI / HTTP transport
   baseline 擴張成 full MCP client certification。
