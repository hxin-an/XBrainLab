...
- scope：
  - Continue product-runtime fallback warning audit for Dataset smart-parse read paths.
  - Prevent real `Study` smart-parse filename query fallback from reading stale controller
    filenames.
- red / focused tests：
  - Added `test_open_smart_parser_refuses_real_study_filename_fallback`.
  - Red gate failed because `_smart_parse_filenames()` let
    `LegacyControllerFallbackUnavailableError` escape when `QueryStateCommand(query="state")`
    returned `None`.
- 做了什麼：
  - `DatasetActionHandler._smart_parse_filenames()` now catches real `Study` legacy fallback
    refusal and shows `Smart Parse Blocked`.
  - Mock / legacy fallback still calls `DatasetController.get_filenames()`.
- validation：
  - Red gate:
...
### 09:23 Label Import fallback warning boundary

- scope：
  - Continue product-runtime fallback warning audit for Dataset label import actions.
  - Prevent real `Study` label-import apply fallback refusal from becoming generic `Failed: ...`
    text.
- red / focused tests：
  - Added `test_import_label_refuses_real_study_controller_fallback`.
  - Red gate failed because `import_label()` caught `LegacyControllerFallbackUnavailableError` only
    in the outer generic exception handler and displayed a generic critical failure.
- 做了什麼：
...
- validation：
  - Red gate:
    ...
    -> failed because warning was not shown and the refusal was logged as generic failure.
  - Focused pass:
    same command -> `1 passed`.
  - DatasetActionHandler regression:
    `QT_QPA_PLATFORM=offscreen poetry run pytest --capture=sys tests/unit/ui/test_ui_misc.py::TestDatasetActionHandler -q`
...
- scope：
  - Continue product-runtime fallback warning audit for Dataset table actions.
  - Prevent real `Study` metadata-update fallback refusal from escaping as a raw legacy fallback
    exception.
- red / focused tests：
  - Added `test_batch_set_refuses_real_study_controller_fallback`.
  - Red gate failed because `_batch_set()` let `LegacyControllerFallbackUnavailableError` escape
    when `UpdateMetadataCommand` returned `None`.
- 做了什麼：
  - `DatasetActionHandler._batch_set()` now catches real `Study` legacy fallback refusal and shows
...
### 09:03 Remove Files fallback warning boundary

- scope：
  - Continue product-runtime fallback warning audit for Dataset table actions.
  - Prevent real `Study` remove-files fallback refusal from escaping as a raw legacy fallback
    exception.
- red / focused tests：
  - Tightened `test_remove_files_refuses_real_study_controller_fallback`.
  - Red gate failed because `_remove_files()` let `LegacyControllerFallbackUnavailableError` escape
    when `RemoveFilesCommand` returned `None`.
- 做了什麼：
  - `DatasetActionHandler._remove_files()` now catches real `Study` legacy fallback refusal and
    shows `Remove Files Blocked`.
  - Mock / legacy fallback still calls `DatasetController.remove_files()`.
- validation：
  - Red gate:
...
- scope：
  - Continue product-runtime fallback warning audit for long-running Training sidebar actions.
  - Prevent real `Study` start-training fallback refusal from being wrapped in generic critical
    error text.
- red / focused tests：
  - Added `test_start_training_refuses_real_study_controller_fallback`.
  - Red gate failed because `TrainingSidebar.start_training_ui_action()` did not show
    `Start Training Blocked` when `TrainCommand` returned `None`.
- 做了什麼：
  - `TrainingSidebar.start_training_ui_action()` now catches real `Study` legacy fallback refusal
    and shows `Start Training Blocked`.
  - Mock / legacy fallback still calls `TrainingController.start_training()`.
- validation：
  - Red gate:
...
- local eval：
  - Not run. This is a Training UI fallback language cleanup under the fast dev gate, not a
    ...
    16GB VRAM pressure.
- 不能宣稱：
  - This does not complete long-running training desktop acceptance, training resource cleanup, or
    all controller fallback debt.

### 08:43 Training Settings fallback warning boundary

- scope：
  - Continue product-runtime fallback warning audit for Training sidebar configuration actions.
  - Prevent real `Study` training-settings fallback refusal from escaping as a raw legacy fallback
    exception.
- red / focused tests：
  - Added `test_training_setting_refuses_real_study_controller_fallback`.
  - Red gate failed because `TrainingSidebar.training_setting()` let
    `LegacyControllerFallbackUnavailableError` escape when the option-side
    `ConfigureTrainingCommand` returned `None`.
- 做了什麼：
  - `TrainingSidebar.training_setting()` now catches real `Study` legacy fallback refusal and shows
    `Training Settings Blocked`.
  - Mock / legacy fallback still calls `TrainingController.set_training_option()`.
- validation：
  - Red gate:
...
- local eval：
  - Not run. This is a Training UI fallback language cleanup under the fast dev gate, not a
    ...
    16GB VRAM pressure.
- 不能宣稱：
  - This does not audit every TrainingSidebar configuration fallback, complete Training Settings
    desktop acceptance, or close all controller fallback debt.

### 08:37 Model Selection fallback warning boundary

- scope：
  - Continue product-runtime fallback warning audit for Training sidebar configuration actions.
  - Prevent real `Study` model-selection fallback refusal from escaping as a raw legacy fallback
    exception.
- red / focused tests：
  - Added `test_select_model_refuses_real_study_controller_fallback`.
  - Red gate failed because `TrainingSidebar.select_model()` let
    `LegacyControllerFallbackUnavailableError` escape when the model-side
    `ConfigureTrainingCommand` returned `None`.
- 做了什麼：
  - `TrainingSidebar.select_model()` now catches real `Study` legacy fallback refusal and shows
    `Model Selection Blocked`.
  - Mock / legacy fallback still calls `TrainingController.set_model_holder()`.
- validation：
  - Red gate:
...
- scope：
  - Continue product-runtime fallback warning audit for Training sidebar data-splitting actions.
  - Prevent real `Study` generate-dataset apply fallback refusal from escaping as a raw legacy
    fallback exception.
- red / focused tests：
  - Added `test_split_data_refuses_real_study_generate_none_controller_fallback`.
  - Red gate failed because `TrainingSidebar.split_data()` let `LegacyControllerFallbackUnavailableError`
    escape when `GenerateDatasetCommand` returned `None`.
- 做了什麼：
  - `TrainingSidebar.split_data()` now catches real `Study` legacy fallback refusal after
    `GenerateDatasetCommand` returns `None` and shows `Data Splitting Blocked`.
  - Mock / legacy fallback still calls `TrainingController.apply_data_splitting()`.
- validation：
  - Red gate:
...
- scope：
  - Continue product-runtime fallback warning audit for Dataset sidebar destructive actions.
  - Prevent real `Study` clear-dataset fallback refusal from being wrapped in a generic critical
    error.
- red / focused tests：
  - Added `test_clear_dataset_refuses_real_study_controller_fallback`.
  - Red gate failed because `DatasetSidebar.clear_dataset()` did not show `Clear Dataset Blocked`.
- 做了什麼：
  - `DatasetSidebar.clear_dataset()` now catches real `Study` legacy fallback refusal and shows
    `Clear Dataset Blocked` with the shared safety message.
  - Mock / legacy fallback still calls `DatasetController.clean_dataset()`.
- validation：
  - Red gate:
...
- local eval：
  - Not run. This is a Dataset UI fallback language cleanup under the fast dev gate, not a
    ...
    16GB VRAM pressure.
- 不能宣稱：
  - This does not audit every DatasetSidebar fallback, complete desktop reset/new-session
    acceptance, or close all controller fallback debt.
...
- scope：
  - Continue read-side fallback audit for Visualization panel average render.
  - Prevent real `Study` stale Average selections from recovering through
    `VisualizationController.get_averaged_record()` when no service payload is available.
- red / focused tests：
  - Added `test_visualization_panel_refuses_real_study_query_none_average_fallback`.
  - Red gate failed because `VisualizationPanel.on_update()` called
    `controller.get_averaged_record()` when `execute_application_command()` returned `None`.
- 做了什麼：
  - `VisualizationPanel.on_update()` now routes legacy averaged-record rendering through
    `run_legacy_controller_fallback()`.
  - real `Study` fallback refusal shows the existing user-facing `No finished runs to average.`
    message and does not read stale controller averaged records.
- validation：
  - Red gate:
    ...
    -> failed on stale `get_averaged_record()`.
  - Focused pass:
    same command -> `1 passed`.
