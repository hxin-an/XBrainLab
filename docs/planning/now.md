...
### Active rollup：Assistant bounded improved baseline

使用者於`2026-08-24`決定停止擴張本輪Assistant修理，把`6df0cc87`累積成果視為明顯優於舊版的
bounded improved baseline。Exact Granite 4.0 Micro 3B v8 report固定revision
`56111ae135df9c53a78c99028e7bc24035a9e979`，結果為36/36 positive、10/10 explicit parameter
origin、5/5 missing-parameter Host guard、20/24 no-action precision與0/7 production-controller
clarification final。20/24與0/7是已知產品限制；本輪不再以prompt、Host semantic router、模型替換或
Assistant coordinator重構追求24/24與7/7。Canonical Stable Assistant promotion gate維持不變，本baseline
不得稱為promotion或handoff-ready。

Rollup branch `integration/assistant-granite-baseline-v1`從PR #49合併後的`main`建立，只依序重放
`b4b1ed3b`、`6ecc74e9`、`fc425eff`、`d53a2ceb`、`6e48a60d`、`09830fdf`與
`6df0cc87`；明確排除失敗的global prompt treatment `bcaeaf9e`與tool-contract treatment
`57059651`。Observable outcome是3B recommended／2B lower-memory Settings、inline runtime setup、
Space不開New Chat、短bubble、Windows中文IME、typed clarification infrastructure及v8 evaluator形成同一個
可延續基準。Scope只允許canonical truth／evidence contract校準與必要無行為整合；Import、18-tool public
contract、confirmation、ApplicationService、其他UI redesign及新模型都不在本slice。

Complexity review：累積XBrainLab production為18 files、`+928/-345/net +583 LOC`；scripts 7 files的
主要增量來自v8 evaluator，另有28個test files與7個docs files。刪除candidate是舊first-run modal，且已在
rollup移除；兩個失敗treatment不進ancestry。Owners before／after不變：AgentManager／
AssistantRuntimeLifecycle擁有UI admission與runtime lifecycle，LLMController協調turn，
ToolAttemptCoordinator擁有admission，PendingInteractionCoordinator只擴充必要cross-turn／TOCTOU receipt，
ApplicationService仍擁有mutation。沒有新增production module、public class或authoritative owner。保留七個
commit邊界作單一rollup PR，避免拆出不可獨立真人驗收的中間產品狀態。

下一步只校準current／decision／validation／developer testing truth，完成Ruff、format、Basedpyright、
MkDocs、完整LLM／Agent與相關UI／walkthrough、自動UI artifact及exact-source non-strict v8 report。Final
report不得低於36/36、10/10、5/5與20/24，七條clarification必須完整記錄；promotion strict gate不修改。
Push frozen exact head並等待PR全部applicable non-skipped checks completed/success後，才交使用者在同一SHA
執行Windows native手測。Settings／inline setup／Space／bubble／Microsoft Pinyin與完整單一安全action是
blocking acceptance；四個已知no-action prompts與多輪補參數失敗只要不比凍結report更差、沒有crash、
資料損失、跨lifecycle receipt或失控重複執行，就記錄但不阻擋。Product source再變或main先推進都使批准
失效；只有使用者回報日期、完整SHA、平台／IME、範圍並明確同意merge後才以PR merge commit進main。
