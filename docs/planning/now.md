...
## 已否決的提案：MOABB Browse & load

老師已於 `2026-08-13` 否決將 MOABB dataset acquisition 串入產品 Import UI；這個提案不再是
target、active roadmap或後續施工項目。先前的worker／download／BIDS materialization設計只保留在
Git history作討論provenance，不能由agent恢復成工作。現有MOABB dataset journeys若仍被另一條
已授權reliability campaign使用，只代表測試資料來源，不代表產品要加入MOABB browser或runtime。

## 已記錄的後續設計：Braindecode Source Vendoring

第三個後續功能的完整實作契約已更新在
[Roadmap 的 deferred Braindecode contract](roadmap.md#deferred-braindecode-release-catalog)。老師要求
architecture source直接內建，因此目標不再是依賴external package加release snapshot，而是保存
Braindecode 1.6.1中57個permissive entries及必要closure、排除四個CC BY-NC models與restricted
shared helper，並在證據完成後移除external `braindecode` runtime dependency。

產品仍只顯示通過XBrainLab training contract的qualified subset；source preservation不等於57個
models都獲產品支援。Pretrained weights、Hub download與datasets不在這個功能內。這仍是deferred
design；目前沒有vendored source、license manifest、checkpoint identity migration或offline package
evidence。必須等正在進行的產品工作合併或明確關閉，再從當時最新`main`開獨立短branch實作。
...
| Braindecode source vendoring | 內建Braindecode 1.6.1的57-entry permissive source subset；產品只顯示qualified models，最終移除external distribution。 | Exact source/license inventory、reproducible generator、upstream parity、versioned checkpoint identity、offline fresh-install和package compliance依Roadmap分段完成。 | Design approved; implementation deferred until current product work merges |
...
- 不實作已被老師否決的MOABB product acquisition；existing dataset campaign不能被解讀成產品功能復活。
...
- 不在目前產品工作完成並合併前開始Braindecode source vendoring；此階段只記錄deferred
  contract，不修改product code、dependency lock或checkpoint schema，也不下載weights。
