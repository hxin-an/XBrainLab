...
| Current classification | Draft PR `#16` validated checkpoint；latest exact-head 的 product gates 已通過，但 aggregate 因 Linux offscreen modal driver 看不到可見 Preview 而 timeout。Same-class driver fallback 與完整 Linux UI shard已本機通過；新 exact-head CI 與 merge尚未完成。 |
