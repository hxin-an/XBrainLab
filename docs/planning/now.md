...
- 最新 MetricsBarChart cleanup 讓 per-class metrics chart close path close 目前 figure、detach /
  `deleteLater()` canvas 並清掉引用；這補齊另一個 Evaluation Matplotlib widget close guard。
...
- 最新 Evaluation panel query cleanup 已讓 real `Study` `EvaluateCommand` result gate display：
  ...
  意外回 `None` 時清成 no-data，不再回讀 stale `EvaluationController.get_plans()`；若 stale
  average/summary selection 仍留在 UI，也不再回讀 stale
...
- 最新 ChatPanel no-data footer wording cleanup 也把 missing-actions fallback 改成
  `Scan a data source to begin`，不再顯示舊式 `Import files to begin`。後續 empty-state
  cleanup 也清掉 ChatPanel 初始 guidance 裡的 `Import EEG files` 字樣。
...
- 最新 class-map preview slice 已讓 BIDS events / CSV / TSV label carrier 在未提供 explicit
  ...
  最新 BIDS sidecar UI replay follow-up 也把 same-directory `events.json`
  `trial_type.Levels` 納入 replay fixture，讓 preview 顯示 `Left hand` / `Right hand` 這類
  user-facing labels；未編輯的 sidecar default 不會被誤寫成 explicit `choices:class_map`。
...
- MCP stdio external-client walkthrough 已新增：
  ...
  - 這支撐 external stdio client path，不代表 Windows release registration 或 HTTP job model；
    HTTP train job status / cancel 已由後續 HTTP baseline 覆蓋。
- MCP Inspector-style release config baseline 已新增：
  ...
  - 這支撐 Inspector CLI / external client release config baseline；HTTP train job status / cancel
    已由後續 HTTP baseline 覆蓋，evaluation / visualization jobs 和 recovery 仍未完成。
- MCP Inspector GUI click-through baseline 已新增：
  ...
  - 這支撐 automated Inspector GUI click-through，不是 human GUI session 或 full MCP client
    certification；HTTP train job status / cancel 已由後續 HTTP baseline 覆蓋，evaluation /
...
- 剩餘非 Goal 1 closure blockers：label import 已能寫入 recipe trace，但尚未成為成熟 import
  ...
  reconciliation、full real-data manual compatibility certification、MCP HTTP evaluation /
  visualization jobs、job persistence / recovery、interactive 3D render、Windows launcher 真人驗收
...
True local LLM ChatPanel long-running tool-command workflow
  ...
  + MCP HTTP evaluation / visualization jobs and recovery hardening
...
9. **Evaluation baseline**
   ...
   - scripted replay 要分 backend replay 和 UI-observable replay；不能只看文字報告就宣稱 UI 行為正確。
     ...
     composer placeholder 縮成 `Ask about EEG workflow`；latest ChatPanel evidence hardening 也讓
     visible text snapshots / `chatpanel.visible_messages` 保存 chat bubble text，不再只靠截圖證明
     clarification / blocked / success messages 可見；resource smoke 會
...
- 不能把 MCP HTTP train job status-cancel baseline 說成 evaluation / visualization jobs、
  persistence / recovery 或 full client certification 已完成。
