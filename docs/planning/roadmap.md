...
- `load_data / attach_labels` 仍作為 legacy compatibility path 存在；agent primary stage
  prompts 已降權它們，但 UI post-load label compatibility 和 MCP/client-facing compatibility
  boundary 仍需繼續盤點，不能宣稱已完全退出產品心智模型。
...
2. **清乾淨 Legacy Command 心智模型**
   ...
   - `load_data / attach_labels` 只保留 service-backed compatibility；agent stage prompt 已降權，
     後續要繼續檢查 UI / MCP/client-facing language 不再把它們當 primary workflow language。
