...
- MCP baseline：real stdio MCP server、stdlib-only client walkthrough、Inspector-compatible
  ...
  click-through artifact 和 local HTTP transport walkthrough 已存在。
...
| Automation Adapters / MCP | CI、eval、batch 和外部 agent 能操作同一套 workflow truth。 | real stdio MCP server、stdlib-only client walkthrough、Inspector config、official Inspector CLI baseline、automated Inspector GUI click-through 和 local HTTP transport baseline 已完成；MCP/headless schema 已標出 legacy data-entry compatibility boundary；Data Interpretation preview choices schema 現在共用 `data_interpretation_choice_schema.py`，所以 MCP / headless / agent 都看得到 recipe remap choices；`tools/call` structured result 現在含 headless adapter/session metadata，避免誤宣稱 desktop UI refresh；stdio / HTTP `train` 先保留 backend precondition truth，只有 backend-ready / enabled long-running training 才回 `long_running_job_required`，不會同步卡住 request；long-running training job progress/cancel/recovery、remote authorization certification 和 full MCP client certification 仍未完成。 | CLI/headless runner 用於 CI / eval / batch；MCP server 暴露同一套 command taxonomy 給 external agents；client 不需安裝 XBrainLab 的 EEG/PyQt/PyTorch 依賴；所有 MCP calls 仍受 capability / autonomy policy 約束。 |
...
7. **完成 Automation Adapters / MCP 驗收**
   ...
   - MCP Inspector GUI click-through 已有 screenshot / visible text / connected tools evidence；
     local HTTP transport 已有 stdlib client walkthrough artifact；後續要補 long-running job
     progress / cancel / recovery 和更完整 client certification。
