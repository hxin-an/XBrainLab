...
- Tool-call benchmark：deterministic suite 已到 `118` thesis-candidate cases；primary
  ...
  artifact 仍是上一輪 `117 / 117`，並有 dashboard breakdown。新增第 `118` case 覆蓋
  downstream-locked `apply_interpretation` wrong-tool temptation；local rerun 尚未更新到 `118`
  cases。這支撐 tool-call benchmark claim，不等於產品 release closure。
...
| Agent Runtime And Tool Surface | assistant 是 workflow operator，透過 state、verification 和 command result 操作軟體。 | local model walkthroughs 和上一輪 `117` case local tool-call benchmark 已支撐 thesis-candidate benchmark claim；deterministic suite 已新增到 `118` cases，補 downstream-locked apply wrong-tool temptation；stage prompt / tool exposure 已把 legacy data entry 降權，Data Interpretation 是 primary data-entry language；長時間 autonomous workflow、UI-routing 和 full recovery behavior 仍未 closure。 | agent tools 遷移到 Data Interpretation + ApplicationService command；State Snapshot、Verification Result、visible response、retry / recovery 都可評分。 |
...
| Validation And Thesis Evidence | 以可重跑 benchmark 證明 agent tool-call 準確率。 | deterministic `118` thesis-candidate cases 已刷新為 `118 / 118`；primary / fallback x3 local rerun 仍是上一輪 `117 / 117`，dashboard 已列 case family、metric、failure taxonomy 和 model comparison；這是 tool-call benchmark evidence，不是整個產品 release evidence。 | benchmark case schema、scorer、runner、artifact、failure taxonomy 和 report 可重現；UI / launcher / MCP / import wizard evidence 不能被 benchmark 分數取代。 |
...
7. **把 Tool-call Benchmark 轉成 Thesis Report Evidence**
   - 目前 deterministic suite 是 `118` cases；local primary/fallback x3 evidence 仍是上一輪
     `117` cases。
