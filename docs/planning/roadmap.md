...
- Tool-call benchmark：deterministic suite 已到 `121` cases，包含 recipe reload
  ...
  artifact 已更新為 `121 / 121`，並有 dashboard breakdown。上一輪 primary
  `microsoft/Phi-4-mini-instruct` 和 fallback `microsoft/Phi-3.5-mini-instruct` 各 `3` 次仍是
  `118 / 118`，尚未覆蓋三個新增 remap cases。scorer 已收緊 blocked intent 下的替代 tool
...
7. **把 Tool-call Benchmark 轉成 Thesis Report Evidence**
   - 目前 deterministic suite 已是 `121 / 121`；local primary x3 和 local fallback x3 仍是上一輪
     `118 / 118`，下一步要重跑 remap-expanded `121` cases。
