...
- Tool-call benchmark：正式 release / thesis evidence 仍是同一 `121` case suite，
  包含 recipe reload `eeg_file_remap` / `label_carrier_remap` 和 missing remap target
  clarification；deterministic、primary `microsoft/Phi-4-mini-instruct` 和 fallback
  `microsoft/Phi-3.5-mini-instruct` artifact 都已更新為 `121 / 121`，local models 各重跑 `3`
  次，並有 dashboard breakdown。最新 fast changed-case slice 已把 source suite 增到 `122`
  cases，新增中文 `幫我貼標籤` missing-input case，但只跑 deterministic `1 / 1` fast gate；
  正式 `122` case local primary / fallback x3 尚未重跑。這支撐已重跑 benchmark slice，不等於
  產品 release closure。
