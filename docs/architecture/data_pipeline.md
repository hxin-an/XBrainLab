...
`.csv`、`.tsv`、`.txt` 和 BIDS-EEG `events.tsv` 是本輪支援的 external carrier；pickle
sidecar 是 blocked，proprietary `.log` 和未知 sidecar 只會標 limited / unsupported，不會被
默默當成可訓練 label。BIDS-EEG review 只把 `events.tsv` 當 label/event carrier；
`participants.tsv` 和 `channels.tsv` 進入 metadata / sidecar evidence，不會污染 class map 或
Epoch defaults。缺 `onset` 是 blocked；缺 `events.json`、缺 duration / end field 或 duration
欄位歧義會成為 warning / action item。這不是 all-modality full BIDS validator support。
...
- `scan_source_path()` 能掃單一 file、folder、BIDS-EEG root，並找到 supported EEG files。
...
- Dataset sidebar 主要入口已改成 `Import file`、`Import folder`、`Import BIDS folder`；BIDS
  入口支援 XBrainLab BIDS-EEG import path，但不代表 all-modality full BIDS validator。
