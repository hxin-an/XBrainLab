...
| Electrode Layout | `QueryStateCommand(state)`、`apply_montage` | Dataset sidebar 在 `Channels` 正下方提供唯一可見入口；它只把現有 channel 對映為 electrode 與 reviewed position，confirmed mutation 走 `ApplyMontageCommand`。BIDS 已發布 layout 時開啟 read-only review；完整 coverage 是 position-dependent consumer 的前提，partial layout 顯示為 limited，但不改變資料軸。 |
