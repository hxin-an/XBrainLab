...
capability policy，第一批 high-value action execution 也已經透過 service-backed
command adapter。
...
- label import、smart parse、channel selection / montage confirmation、split dialog internals、
  model / training setting dialog submit、evaluation / visualization query actions 尚未改成
...
- panel 內建 header：`XBrainLab Assistant`、面向使用者的 workflow 說明、local model status
  chip、backend workflow stage chip、next-step chip、Retry / Clear / Stop / Settings controls。
...
- `Coder / Local / Multi` 這類開發者語言改成 `Workflow guide`、`Local model`、
  `Single step`、`Auto steps`。
...
- 部分 dialog / query workflow action execution 仍是 controller direct-call；核心 import /
  preprocess / epoch / training start-stop / reset 已先接 service adapter。
