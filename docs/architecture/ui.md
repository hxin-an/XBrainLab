...
- panel 內建簡潔 header：`XBrainLab Assistant`、面向使用者的 workflow 說明和 `Options`。
  backend workflow stage / next step 改為 header 下方的單句 guidance，不再是 chip dump。
  `Retry` / `Clear` 移到底部 composer footer；local model / backend detail 放在底部低干擾
  runtime status、main status bar、tooltip 或 settings。
...
- `Coder / Local / Multi` 這類開發者語言改成 `Workflow guide`、`Local model`、
  `Single step`、`Auto steps`，並移進 Options 或底部低干擾 status。
...
- `Retry` 沒有上一則 request 時必須 disabled；程式直接呼叫時也只顯示 footer/status notice，
