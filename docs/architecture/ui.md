...
| `XBrainLab/ui/components/info_panel_service.py` | aggregate info panel 的集中更新服務；成功資料變更由 `data_changed` / `preprocess_changed` 驅動。 |
...
與 shared status refresh 不再散在 `MainWindow` 內，但 refresh 來源仍有兩種：使用者切換頁面，
