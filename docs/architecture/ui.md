...
五個 workflow panel constructor 也不再把 `parent.study.get_controller(...)` 當 real `Study`
fallback；MainWindow product wiring 會注入 controllers，mock / legacy contexts 才透過
`get_legacy_controller_from_study()` 取得 controller。AgentManager 初始化時也不再直接從 real
`Study` 取 preprocess controller；該 controller 只保留給 montage legacy apply fallback。
...
Visualization sidebar montage setup now uses `QueryStateCommand(query="state")` for channel-name
dialog defaults. `VisualizationController.get_channel_names()` is limited to mock / legacy query
fallback. AgentManager uses the same state-query source before opening the assistant-driven
montage picker, so product UI no longer reads `study.epoch_data` directly for dialog channel
choices. Montage apply still uses `ApplyMontageCommand` with confirmed positions.
...
direct `study.get_controller(...)` lookup、`controller.study.get_controller(...)` observer wiring
fallback、mutating command `refresh=False`，
...
boundary，不代表 controller 已完全退場。最新 guard 已移除 `main_window.py` 的 direct
`study.get_controller(...)` 例外；剩餘 panel bootstrap controller lookup 必須待在
`legacy_controller_bootstrap.py` 這類具名 legacy/fallback helper 內。
2026-05-13 follow-up 又把 guarded UI product smokes 納入相同證據邊界：
`tests/integration/ui/test_product_walkthrough.py` 和 `test_epoch_runtime.py` 不可用 direct
mutable `Study` state read 或 `study.get_datasets_generator()` 作為 product-success evidence；
成功條件要查 `QueryStateCommand` / command diagnostics 或 UI-visible state。這是測試證據
guard，不代表所有非 product-smoke integration tests 都已完成同樣清理。
...
- label import dialog planning、montage picker / matching、panel read-only population 與 panel
  constructor controller adapters 還保留 controller / UI-request compatibility path；實際 label
  import、smart parse、montage confirmation apply 已接 service command adapter。
...
- Guarded UI product smokes 已不再以 direct mutable `Study` state read 作為成功證據；其他
  integration suites 的 fixture/setup 型 direct state access 還需要分批判讀，不能一概當作
  product acceptance。
