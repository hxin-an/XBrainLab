...
2026-05-13 follow-up 又把 guarded UI product smokes / real-tools evidence 納入相同證據邊界：
`tests/integration/ui/test_product_walkthrough.py`、`test_epoch_runtime.py`、
`test_real_tools_e2e.py` 和 guarded real-tools integration suites 不可用 direct mutable
`Study` state read 或 `study.get_datasets_generator()` 作為 product-success evidence；成功條件
要查 `QueryStateCommand` / command diagnostics、command-owned objects 或 UI-visible state。
