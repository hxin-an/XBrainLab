...
panel。因此 tab-switch refresh 的 panel mapping 不再散在 `MainWindow` 內，但 refresh
來源仍有兩種：使用者切換頁面，以及 backend event 經 observer bridge 觸發。
後續 observer cleanup 已把單純的 `event -> update_panel()` bridge 先收斂到
`BasePanel.refresh_from_observer()`；該 helper 再委派 `refresh_coordinator.refresh_panel()`。
這保留 backend observer event 語意，但把 safe no-arg panel refresh call 放進同一個
coordinator 邊界。
...
| Montage confirmation apply | `apply_montage` | confirmed positions use `ApplyMontageCommand`; montage picker / matching remains UI request path |
...
單純的 observer-driven panel refresh 應使用 `BasePanel.refresh_from_observer()`，
不要直接把 event handler 接到 `update_panel()`。需要特殊語意的 event，例如 import-finished
handler、TrainingPanel 的 start/stop handler 或 live training update loop，仍可接自己的 handler。
