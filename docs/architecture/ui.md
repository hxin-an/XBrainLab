...
目前 `MainWindow` 是 product runtime 取得五個 workflow controllers 的 central wiring point。
...
legacy adapter，但直接 controller lookup 已開始被集中與 guard。
...
controllers 建立 observer bridge，且 mock / legacy adapter 仍可使用 controller fallback。因此後續
