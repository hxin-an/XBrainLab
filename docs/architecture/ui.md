...
| --- | --- | --- |
| Dataset data interpretation | `scan_source` / `preview_interpretation` / `validate_interpretation` / `apply_interpretation` | Dataset sidebar `Interpret Data Source` runs scan -> preview dialog -> validate -> confirm/apply via service; mock / unsupported fallback can still call legacy `LoadDataCommand` / `DatasetController.import_files()` |
