...
| Saliency admission | Training terminal path 只發布 metrics；visible `Compute Saliency` 送出明確的 generation/run-bound operation，成功 publication 後才開啟 attribution views。 | 沒有按鈕動作就沒有 saliency 成功主張；Map / Spectrogram source tests 不取代 15-dataset screenshots、scientific validity 或 native 3D acceptance。 |
