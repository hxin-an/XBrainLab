...
- 顯示 local 狀態；Gemini/API runtime code 仍殘留，但一般產品 UI 已用
  `XBRAINLAB_SHOW_LEGACY_REMOTE_LLM=1` 隔離，未設定時不顯示 remote runtime 選項。
...
目前程式碼仍存在 API / Gemini backend 與切換邏輯。這是 current code state，不是未來設計目標。
2026-05-02 product audit follow-up 已把一般產品入口隔離到
`XBRAINLAB_SHOW_LEGACY_REMOTE_LLM=1` 後面：ChatPanel model menu、ModelSettingsDialog remote
section、AgentManager startup 都不會在預設產品路徑顯示或啟動 Gemini。code path 尚未刪除，
後續仍要 cleanup。
...
- mapped workflow tools 會直接把 `CommandResult` 轉成 `ToolCommandResult`，目前包含
  ...
  `configure_training`、`start_training`、`clear_dataset`。
...
- `load_data` 仍保留 legacy real-tool path，因為它處理 directory expansion / file safety；
  `set_montage` 仍走 UI confirmation request；`switch_panel` 仍是 UI routing request；
  `list_files` / `get_dataset_info` 仍是 read-only / inspection path。
...
- API / Gemini code path 的移除尚未完成；一般產品入口已隔離，但程式碼仍需後續刪減。
