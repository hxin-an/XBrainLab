...
`Study` 狀態。2026-05-11 command-spine cleanup 後，product runtime real tools 不再使用
`BackendFacade`；mapped workflow tools 和 real-tool compatibility implementations 都以
command result / command query 為準。
...
- `BackendFacade` 仍存在於 legacy compatibility layer，但不再是 product runtime path。
...
- `BackendFacade` 可以暫時保留為 legacy compatibility adapter，但 assistant / script product
  path 應直接使用 ApplicationService / Command API。
