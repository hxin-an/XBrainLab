...
`SaveInterpretationRecipeCommand` 寫入 recipe。label import migration、完整 MCP server 和
local LLM 真實 tool-call
...
- Data Interpretation backend command baseline、agent tool exposure、Dataset panel main
  ...
  和第一版 UI-observable replay artifact 已進入新心智模型；label import migration、完整 MCP
  server 和 local LLM 真實 tool-call eval 尚未完成。
