...
| Test truth | product-success tests 已開始被 guard 約束，不能用 facade、legacy fallback、direct mutable `Study` state、positive controller lookup、no-crash 字串當成功證據；MCP product status/progress path 也被 guard 禁止直接讀 mutable `Study` state。 |
...
| MCP direct Study state guard | 2026-05-13 architecture guard unit / architecture compliance PASS。 | 支撐 MCP product status/progress code 不能新增 `service.study.trainer` 這類 direct mutable `Study` state read，除非在 explicit legacy/fallback helper。 |
...
| quality dashboard | 2026-05-13 22:19:04 UTC+08:00 `fast` PASS。 | engineering health，不是 release approval。 |
