...
- 最新 real-tools evidence cleanup 把 LLM real-tool integration tests 從 direct controller reads /
  ...
  當成功證據。
- 最新 UI product-smoke evidence cleanup 把 `test_product_walkthrough.py` 和
  `test_epoch_runtime.py` 的 success assertions 從 direct mutable `Study` state read 改成
  `QueryStateCommand` / UI-visible state；architecture guard 也新增 product UI smoke direct
  Study-state rule，避免 product-success tests 重新用 `study.epoch_data`、`study.datasets` 或
  `study.get_datasets_generator()` 當成功證據。
...
| UI product-smoke query truth | 2026-05-13 focused product walkthrough / epoch runtime suites PASS。 | 支撐 guarded UI product smokes 不再以 mutable `Study` state read 當成功證據；不等於 full zero-controller UI。 |
...
| quality dashboard | 2026-05-13 13:39:20 UTC+08:00 `fast` PASS。 | engineering health，不是 release approval。 |
