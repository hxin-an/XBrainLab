...
Data」的 service-backed compatibility path，但尚未整合進 Data Interpretation recipe；完整
MCP server 和 local LLM 真實 tool-call accuracy 也尚未完成，舊 `load_data / attach_labels`
仍不能宣稱已完全退出產品心智模型。
...
- Data Interpretation backend command baseline、agent tool exposure、Dataset panel main
  import entry、recipe save option、headless/MCP-ready command schema、deterministic eval cases
  和第一版 UI-observable replay artifact 已進入新心智模型；label import 目前是
  service-backed compatibility path，尚未整合進 Data Interpretation recipe；完整 MCP server
...
4. 產品主線穩定後，建立 local LLM primary / fallback tool-call accuracy run、score report 和
   failure taxonomy。
