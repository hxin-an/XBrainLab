...
| Delivery state | Product-only extraction 已 push 並建立 draft PR `#16`。最近 exact-head CI 的 platform、unit、其餘 integration 與 required public multi-dataset gates 已通過；aggregate 仍因 Linux UI driver 在 Loading→Preview modal 交接後只查 `activeModalWidget()` 而超時退件。Driver 現在只在 active modal 缺失時接受唯一可見 modal surface；本機完整 Linux UI shard 已回復通過。仍須 push 新 exact head、等待所有 non-skipped checks completed/success 並經 PR merge，才可成為 `main`。 |
