...
| preprocess validation command-load setup | 2026-05-13 focused preprocess validation / architecture guard PASS。 | 支撐 `test_preprocess_validation.py` 的 synthetic FIF fixture load 成功證據改由 `LoadDataCommand` / `QueryStateCommand` state 驗證，且 event regression 使用 exact `left/right` evidence；preprocess edge-case assertions 仍是 controller-level regression，不被誇大成完整 product smoke。 |
...
| MCP direct Study state guard | 2026-05-13 architecture guard unit / architecture compliance PASS。 | 支撐 MCP product status/progress code 不能新增 `service.study.trainer` 這類 direct mutable `Study` state read，也不能走 `service.study.get_controller(...)` 這類 controller lookup bypass，除非在 explicit legacy/fallback helper。 |
...
| quality dashboard | 2026-05-13 22:29:42 UTC+08:00 `fast` PASS。 | engineering health，不是 release approval。 |
