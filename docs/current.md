...
| Data Interpretation label semantics | 2026-05-14 focused backend unit suites PASS。 | 支撐 reviewed event-code label files、interval end/stop/offset duration、run-dependent internal event mapping、recipe/state snapshot preservation 的 backend contract；不是 Match Labels UX approval。 |
| Data Import screenshots | `artifacts/ui/data-import-wizard-steps/` 保留 canonical wizard screenshots 和四個 loaded-label placement panels。 | 支撐目前畫面判讀；不是 final Match Labels / Review and Import UX approval。 |
