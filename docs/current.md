...
target clarification；local primary / fallback x3 尚未對新的 `121` suite 重跑。這支撐目前已
重跑 benchmark slice；仍不等於 UI usability、Windows launcher、雙螢幕 / DPI、長時間桌面
session 或 product completion。後續
...
recipe reload remap、label carrier remap 和 missing remap target clarification。這仍不是完整
conflict editor、複雜 anchor reconciliation，且 local primary / fallback x3 尚未針對新的
`121` case suite 重跑；`artifacts/agent_evals/dashboard.md` 已明確顯示 deterministic `121`
與 local `118` 的 coverage mismatch。
...
- thesis protocol 已建立 split artifact schema、split audit helper 和 validator script；正式
  ...
  均為 `118 / 118` pass，並有 dashboard breakdown；最新 deterministic artifact 則是
  `121 / 121`，新增 recipe remap cases。新的 `121` suite 尚未重跑 primary / fallback local x3。
  這支撐已重跑 tool-call benchmark slice，不支撐 UI / launcher 產品完成 claim。
...
- tool-call eval 已有 deterministic baseline 和 primary / fallback 真 local model runner；最新
  deterministic artifact 已刷新為 `121 / 121`，包含 recipe reload
  `eeg_file_remap` / `label_carrier_remap` 和 missing remap target clarification。上一輪
  primary / fallback 真 local model x3 artifact 仍是 `118 / 118`，尚未覆蓋新的 remap-expanded
  `121` case suite，所以不能把 local LLM thesis-candidate claim 擴張到這三個新增 cases。
...
5. 繼續根據 human-like walkthrough screenshots 做 UI polish；第一輪已處理 Data Interpretation
   ...
   composition 和整體產品感。
6. 重跑 primary / fallback local LLM x3 的 `121` case remap-expanded tool-call dashboard；
   在重跑前只能宣稱 deterministic `121 / 121` 與上一輪 local `118 / 118`，不要把它擴張成
   UI / launcher 完成 claim。
