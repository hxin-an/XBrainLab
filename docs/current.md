...
這類常見格式名稱在 product-width dialog 中完整可見；backend recipe choices 仍保存原始欄位值。最新
...
`DatasetController.get_loaded_data_list()`。後續 guard slice 已讓 `Add Labels to Loaded Data` 在沒有 loaded data 時
...
- product runtime mutating workflow 不應 silent fallback 到 controller mutation。現有
  ...
  `get_model_summary_str()`。這個 object payload flag 是 UI-only：automation / MCP `evaluate`
