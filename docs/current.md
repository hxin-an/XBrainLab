...
`integration/import-training-montage-polish-v1` integration candidate。`ApplicationService / Command API`
spine 已存在，本輪正在收斂 import terminal feedback、BIDS montage preparation、Epoch baseline
interaction、Training resource preview / optimizer contract 與 Visualization capability controls；
仍須把同一候選 commit 推送並取得 exact-head CI 成功，才可交給使用者做 Windows 人工驗收。
...
| 項目 | Current truth |
| --- | --- |
| Active worktree | `integration/import-training-montage-polish-v1`；候選收斂中，repo-root `settings.json` 是不得納入版本控制的使用者本機設定。 |
| Product baseline | `main` |
| Current candidate | `integration/import-training-montage-polish-v1` 的下一個 pushed exact-head commit；不是 release。 |
...
| EEG workflow baseline / current polish | BIDS subject selection、curated Braindecode catalog、test curve、cross-fold summary 與 detached Saliency Normalize 已進入 `main`。目前候選另加入 generation-bound BIDS montage preparation、Training draft resource preview、有限 optimizer contract、Epoch baseline toggle 與位置能力提示。 | Current polish 尚未合併；Braindecode catalog 不是「支援 Braindecode 全模型」，cross-fold summary 不是 scientific model comparison，Normalize 不改原始 attribution，2-D montage 也不代表可用 3-D geometry。 |
