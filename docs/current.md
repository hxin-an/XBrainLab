...
`microsoft/Phi-4-mini-instruct` 為 `51 / 54` pass、fallback
`microsoft/Phi-3.5-mini-instruct` 為 `50 / 54` pass；這是可重跑的真模型 engineering
...
eval 已重跑 `54` cases x `3`：primary `51 / 54`、fallback `50 / 54`。剩餘 blocker 是
saliency / visualization UI-route substitute、fallback invalid event recovery、
...
- tool-call eval 已有 deterministic baseline 和 primary / fallback 真 local model runner；最新
  primary `51 / 54`、fallback `50 / 54` 仍只能作 engineering evidence，不能宣稱
  thesis-ready，因為 case 數仍不足 `100` 且仍有 saliency / visualization、invalid event、
