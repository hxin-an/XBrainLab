...
XBrainLab 的唯一 product baseline 是 `main@b3a87e3996585ebb09ae46335da82d234ae70249`；
EEG workflow PR #12 與 GPT-5.6 guidance PR #13 已合併。現在不再指定長期 integration
candidate；每項工作從最新 `main` 建立短 task branch，以實際 checkout 與 PR exact head 為準。
目前 validation control plane 是 local PR candidate，尚未取得 exact-head CI，也不提升產品或
Windows handoff 狀態。
...
| EEG workflow baseline | BIDS import 可在正式 scan 前列出 subjects、sessions、tasks、runs，且只掃 selected subjects；Training 提供 curated Braindecode model catalog 並發布 test accuracy curve；Evaluation / Visualization 使用一基索引的 Fold / Run 語言，只有 backend 證明 test masks 與 cohort 相容時才提供 `All Folds` summary；Saliency Normalize 只改 detached render data。 | 已由 PR #12 進入 `main`；Braindecode catalog 仍不是「支援 Braindecode 全模型」，cross-fold summary 不是 scientific model comparison，Normalize 不改原始 attribution，且不取代 Windows acceptance。 |
