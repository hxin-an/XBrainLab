...
| Data Interpretation | `scan -> preview -> validate -> apply -> recipe` baseline 已存在；current UX checkpoint includes canonical wizard screenshots, internal EEG label evidence, loaded-label placement evidence for EEG event order / label time / label interval / label event code, BIDS-like events review, blocked conversion fallback, and backend-generated epoch handoff evidence. | 還不是 final import system；P300/ERP hierarchy、SSVEP/c-VEP semantics、clinical long recordings、XDF/LSL、OpenBCI/BrainFlow、MOABB adapters、proprietary logs、nested unknown MAT schemas、pickle、user Python converters 都不能誇大。 |
...
| Data Interpretation label semantics | 2026-05-14 focused backend unit suites PASS。 | 支撐 reviewed target EEG event selection、event-code label files、interval end/stop/offset duration、run-dependent internal event mapping、recipe/state snapshot preservation、epoch handoff 的 backend contract；不是 human Data Import acceptance。 |
| Data Import screenshots | `artifacts/ui/data-import-wizard-steps/` 保留 canonical wizard screenshots、conversion fallback/example、BIDS-like events、Review and Import 三狀態，以及四個 loaded-label placement panels。 | 支撐目前畫面判讀；不是 human Windows click-through acceptance，也不是所有非目標格式的支援聲明。 |
