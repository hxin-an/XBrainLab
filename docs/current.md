...
MCP external-client 驗收尚未完成，舊 `load_data / attach_labels` 仍不能宣稱已完全退出產品
心智模型。
...
- Data Interpretation backend command baseline、agent tool exposure、Dataset panel main
  ...
  label import 目前是 service-backed compatibility path，尚未整合進 Data Interpretation
  recipe；MCP external-client / Inspector walkthrough 尚未完成。
