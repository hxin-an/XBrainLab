...
- true local LLM ChatPanel 已有一輪真模型 UI 回覆 walkthrough；multi-turn tool-command / 長時間
  workflow walkthrough 仍未完成。
- local LLM primary / fallback tool-call runner 已有 `100` thesis-candidate cases x `3`
  score report；primary / fallback 都是 `100 / 100` pass。這支撐 tool-call benchmark
