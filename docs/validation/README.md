...
| `poetry run pytest --capture=sys tests/unit/test_architecture_compliance.py -q` | `102 passed`. | Architecture guard now rejects `scripts/dev/verify_real_tools.py` using `study.loaded_data_list`, `study.datasets`, `study.is_training()`, or `study.stop_training()` as product verifier truth, while allowing `QueryStateCommand` / `StopTrainingCommand`. | Runtime success of the full verifier. | Keep headless product scripts on ApplicationService command/query truth. |
