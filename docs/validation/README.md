...
| Agent requested-intent boundary 是否仍只看 result exists | [LLM agent intent-boundary exact result evidence](#2026-05-14-llm-agent-intent-boundary-exact-result-evidence-checkpoint) | Controller tests 檢查 blocked requested workflow 的 `ToolCommandResult` type、tool/command name、blocked reason、message、capability 和 state；guard 擋回 generic non-`None` result assertion。 | Full assistant UX acceptance、tool-call benchmark accuracy、or local-model behavior。 |
| Agent confirmation gate 是否仍只看 pending object exists | [LLM agent confirmation exact prompt evidence](#2026-05-14-llm-agent-confirmation-exact-prompt-evidence-checkpoint) | Controller confirmation tests 檢查 exact pending tuple、status update、`confirm_action` payload；guard 擋回 generic `_pending_confirmation is not None`。 | Full HITL UX approval、local-model behavior、or end-to-end chat acceptance。 |
| Controller integration init 是否仍只看 component exists | [LLM controller integration exact contract evidence](#2026-05-14-llm-controller-integration-exact-contract-evidence-checkpoint) | Controller init test 檢查 component types、assembler wiring、exact registered real-tool order、verifier schema keys；guard 擋回 generic component non-`None` / tool-count assertion。 | Full assistant runtime acceptance、local-model behavior、or UI chat UX。 |
| Agent tool definitions 是否仍只看 name/description 非空 | [LLM tool-definition exact schema evidence](#2026-05-14-llm-tool-definition-exact-schema-evidence-checkpoint) | Tool-definition tests 檢查 exact tool names、schema properties、required fields、description markers；guard 擋回 generic `len(...) > 0`。 | Full assistant UX acceptance、runtime tool execution, or tool-call benchmark accuracy。 |
...
| Headless dev smokes 是否仍直接讀 Study state | [Headless real-tools query evidence](#2026-05-14-headless-real-tools-query-evidence-checkpoint) | `scripts/dev/verify_real_tools.py` and `scripts/dev/run_public_cross_source_training_smoke.py` now use command/query truth for dataset/training state; architecture guard rejects direct `Study` reads / training control from returning. | A full real-tools integration test, human assistant acceptance, or long training quality. |
...
| BIDS-EEG import 是否真的接到 Epoch | [Data Import BIDS-EEG / Epoch product checkpoint](#2026-05-14-data-import-bids-eeg-epoch-product-checkpoint) | BIDS root scan、selected scope、events.tsv / events.json Levels、channels.tsv bad-channel evidence、recipe、epoch handoff、CreateEpoch defaults 有 focused tests、product smoke 和 JSON evidence。 | all-modality full BIDS validator、human Windows acceptance、or non-goal format support。 |
...
## 2026-05-14 LLM Agent Intent-Boundary Exact Result Evidence Checkpoint

This test-quality slice tightened the assistant controller tests for requested workflow intent.
It does not certify local-model behavior or the full chat UX; it proves that when the user asks
for train / visualize / saliency and the model chooses a substitute tool, the controller returns
an exact structured block instead of merely producing a non-empty result.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| `poetry run pytest --capture=sys tests/unit/llm/agent/test_controller.py tests/unit/test_architecture_compliance.py -q` | `172 passed`. | Requested-intent boundary tests check `ToolCommandResult` type, exact selected tool, requested command, blocked reason, message, capability, and state. | Full assistant UX acceptance, local-model tool-call quality, or end-to-end chat behavior. | Continue replacing weak agent tests only when the replacement checks concrete command/result/state semantics. |
| `poetry run python tests/architecture_compliance.py` | `Architecture compliant!`. | Architecture guard now rejects generic non-`None` result assertions in the requested-intent controller tests. | Semantic proof for every future assistant controller path. | Keep this guard narrow unless additional weak-evidence patterns are first replaced. |
...
## 2026-05-14 LLM Tool-Definition Exact Schema Evidence Checkpoint

This test-quality slice tightened the agent tool-definition contract tests. It does not prove
runtime execution or local-model behavior; it prevents schema drift from being hidden behind
generic "non-empty name/description" assertions.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| `poetry run pytest --capture=sys tests/unit/llm/tools/test_definitions.py tests/unit/test_architecture_compliance.py -q` | `263 passed`. | Tool-definition tests check exact tool names, schema property order, required fields, and description markers for the agent-visible tool contract. | Runtime tool execution, assistant UX acceptance, local-model quality, or tool-call benchmark accuracy. | Keep future schema changes explicit by updating the contract table and related product docs. |
...
## 2026-05-14 LLM Controller Integration Exact Contract Evidence Checkpoint

This test-quality slice tightened the controller initialization integration test. It does not
certify local-model runtime or chat UX; it verifies the controller wires the real tool surface into
the registry, context assembler, and verifier schema contract.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| `poetry run pytest --capture=sys tests/unit/llm/agent/test_controller_integration.py tests/unit/test_architecture_compliance.py -q` | `116 passed`. | Controller initialization checks component types, assembler wiring, exact registered real-tool order including `query_state`, and verifier schema keys for the agent-visible tool surface. | Local-model runtime, full assistant UX, or UI chat acceptance. | Keep exact tool-order changes paired with tool-definition and tool-surface updates. |
| `poetry run python tests/architecture_compliance.py` | `Architecture compliant!`. | Architecture guard now rejects generic component non-`None` and tool-count assertions in the controller integration initialization test. | Semantic proof for all controller paths. | Keep controller integration evidence tied to concrete wiring and schema contracts. |
| Focused `ruff check` / `ruff format --check` / `basedpyright`, plus `poetry run mkdocs build --strict`, `poetry run python scripts/dev/update_quality_dashboard.py`, and `git diff --check` | PASS / PASS / `0 errors`; docs, dashboard, and diff PASS. | Changed tests/docs remain lint/type/docs/dashboard/diff clean after validation. | Product completion or human Windows acceptance. | Keep generated dashboard artifacts as evidence output, not canonical docs truth. |
...
confirmation dialog or full human-in-the-loop UX; it proves the controller stores and emits the
exact confirmation contract that the UI consumes.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| `poetry run pytest --capture=sys tests/unit/llm/agent/test_controller.py tests/unit/test_architecture_compliance.py -q` | `176 passed`. | Static and backend-dynamic confirmation paths check exact pending tuple, status update, and `confirm_action` payload with tool name, params, and description. | Full confirmation dialog UX approval, local-model behavior, or end-to-end chat acceptance. | Pair with UI/HITL artifact only when visible confirmation UX changes. |
| `poetry run python tests/architecture_compliance.py` | `Architecture compliant!`. | Architecture guard now rejects generic `_pending_confirmation is not None` assertions in controller confirmation tests. | Semantic proof for all assistant controller behavior. | Keep confirmation tests payload-specific. |
| Focused `ruff check` / `ruff format --check` / `basedpyright`, plus `poetry run mkdocs build --strict`, `poetry run python scripts/dev/update_quality_dashboard.py`, and `git diff --check` | PASS / PASS / `0 errors`; docs, dashboard, and diff PASS. | Changed tests/docs remain lint/type/docs/dashboard/diff clean after validation. | Product completion or human Windows acceptance. | Keep generated dashboard artifacts as evidence output, not canonical docs truth. |
...
dev verifier and public cross-source training smoke so their success/failure decisions use
ApplicationService command/query truth instead of direct mutable `Study` fields.
...
| Focused source audit of `scripts/dev/run_public_cross_source_training_smoke.py` | Direct checks of `study.datasets`, `study.trainer`, `study.generate_plan()`, and `study.train(...)` were replaced with `GenerateDatasetCommand` state, `TrainCommand(confirmed=True, interactive=False)`, and `QueryStateCommand(query="training_history", include_objects=True)`. | The public smoke no longer uses mutable Study fields or direct Study training methods as product evidence. | Scientific training quality, long training stability, or fixture availability. | Prefer maintained integration tests for automated gates; run the script only when public fixtures are present. |
| `poetry run pytest --capture=sys tests/unit/test_architecture_compliance.py -q` | `103 passed`. | Architecture guard now rejects headless product scripts using `study.loaded_data_list`, `study.datasets`, `study.trainer`, `study.generate_plan()`, `study.train(...)`, `study.is_training()`, or `study.stop_training()` as verifier truth, while allowing `QueryStateCommand`, `TrainCommand`, and `StopTrainingCommand`. | Runtime success of the full verifier or public smoke. | Keep headless product scripts on ApplicationService command/query truth. |
| `poetry run pytest --capture=sys tests/unit/test_architecture_compliance.py tests/unit/scripts/test_run_public_cross_source_training_smoke.py -q` | `105 passed`. | The guard and public-smoke report rendering tests remain green after the script cleanup. | Full public fixture runtime. | Run integration public-smoke tests only when the fixture set is available and runtime cost is intended. |
...
This test-quality slice tightened the focused dialog acceptance baseline. It does not redesign any
dialog; it makes the existing offscreen OK-button tests assert concrete dialog contracts.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| `QT_QPA_PLATFORM=offscreen MNE_DONTWRITE_HOME=true poetry run pytest --capture=sys tests/integration/ui/test_dialog_acceptance.py -q` | `4 passed`. | Dialog acceptance coverage checks Ok/Cancel button-box contract, enabled OK action, accepted state, exact mapping/event/epoch payloads, and typed training option values. | Human dialog UX approval, screenshot quality, or Data Import UX redesign. | Keep dialog tests payload-specific and avoid generic OK-exists evidence. |
| Focused `ruff check` / `ruff format --check` / `basedpyright`, plus `poetry run mkdocs build --strict` and `git diff --check` | PASS / PASS / `0 errors`; docs and diff PASS. | Changed files remain lint/type/docs/diff clean after validation. | Product completion. | Pair future dialog UX claims with screenshot or human evidence. |
...
## 2026-05-14 Data Import BIDS-EEG / Epoch Product Checkpoint

This Data Import slice covers the agreed Tier 1 / Tier 2 classes only: GDF / BNCI-style internal
and external labels, XBrainLab BIDS-EEG events, generic internal EEG events / annotations, and
MAT / CSV / TSV / TXT external label carriers. BIDS support here means EEG raw files under a BIDS
layout, selected scope separate from scan location, `participants.tsv` metadata when present,
`events.tsv` with onset / duration / trial_type / value, `events.json` Levels for class names, and
`channels.tsv` status evidence. It does not claim all-modality full BIDS validation, P300/ERP
hierarchy, SSVEP/c-VEP semantics, clinical long recordings, XDF/LSL synchronization,
OpenBCI/BrainFlow semantics, MOABB adapters, proprietary logs, pickle, or executable user Python
converters.

| Command / audit | Result | Claim supported | Claim not supported | Follow-up |
| --- | --- | --- | --- | --- |
| Focused backend Data Interpretation suites | `144 passed` | Selected EEG scope versus scan location, external labels outside the EEG folder, selected target EEG events, BIDS-EEG warnings/blockers, unsupported sidecar boundaries, run-dependent internal event mapping, recipe/state snapshot, epoch handoff, and label apply semantics are covered by unit tests. | All possible EEG datasets, scientific correctness, or human Windows acceptance. | Keep adding concrete fixtures when a new supported format class is added. |
| Focused UI/dialog/replay/product-smoke tests | `277 passed` in the combined focused run; `9 passed` for the ApplicationService workflow file. | Load Labels, Match Labels, Review and Import visible behavior, footer visibility, conversion fallback, replayed product flow, and BIDS import -> apply -> CreateEpoch product smoke remain green. | Visual approval by itself or manual desktop acceptance. | Pair UI tests with screenshot review and human click-through before release claims. |
| Screenshot/product smoke scripts | PASS | Canonical wizard screenshots, four loaded-label placement panels, conversion fallback/examples, BIDS-EEG state, Review and Import states, Epoch after BIDS import, and backend-generated epoch handoff evidence were regenerated. | Full BIDS support beyond the stated EEG boundary or exhaustive non-goal format support. | Keep `artifacts/ui/data-import-wizard-steps/README.md` as the screenshot index. |
| `ruff check` / `ruff format --check` / changed-surface `basedpyright` / `mkdocs build --strict` / architecture guards / `git diff --check` | PASS / PASS / `0 errors, 0 warnings, 0 notes` / PASS / `Architecture compliant!`, guard unit `93 passed` / PASS | Changed code, tests, docs, and artifacts are lint/type/docs/architecture/whitespace clean. | Product complete, full-repo type-baseline cleanup, or human Windows click-through acceptance. | Fast dashboard currently depends on the per-worktree Poetry env having UI/test deps installed; run it from a hydrated env before using it as release evidence. |
