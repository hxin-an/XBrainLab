...
全數 nonblank，visible text raw tool / schema / traceback / selected internal snake_case command
leakage findings 為 `0`。最新
...
最新 offscreen artifact 顯示 resource smoke `passed=True`、RSS growth `231556 KB` /
...
capture 中載入資料後 table widget 直接貼齊 sidebar。最新 human-like walkthrough capture
script 也會先讓 MainWindow show 後的 startup geometry recovery timer 跑完，再把 window
恢復到 `1280x800` artifact 尺寸；最新 `06-interpretation-applied.png` 是 `1280x800`，
JSON 記錄 loaded Dataset table `widget_width=1020`、`header_length=993`、
`viewport_width=994`、`right_gap_to_boundary=0`。這是 automated layout evidence，仍需真人
桌面驗收判斷整體視覺品質。
...
- Claim boundary：this supports the 118-case local tool-call benchmark claim for the saved suite.
  ...
  long-running job automation, or full product completion. The apply-lock case may still parse as a
...
config 或 product completion；HTTP transport 已由後續 2026-05-06 baseline 補上。
...
它本身仍不支撐 MCP Inspector GUI click-through、Windows release registration / config、
long-running training tool through MCP、或 product completion；Inspector release config、GUI
walkthrough 和 HTTP transport baseline 已由後續 artifacts 補上。
...
- 新增 `XBrainLab.mcp.http_server`：
  - 使用 Python stdlib `ThreadingHTTPServer`，不把 PyQt / EEG / PyTorch dependencies 推給
    client process。
  - `GET /health` 回傳 local headless session state、transport 和 UI-refresh boundary。
  - `POST /mcp` 接 MCP JSON-RPC request，委派到同一個 `MCPServer(transport="http")` /
  ...
  - Terminal job status is preserved: cancelled / completed jobs remain terminal when a later train
    run starts in the same HTTP session.
...
- 新增 `scripts/dev/capture_mcp_http_walkthrough.py`：
  ...
  - walkthrough 覆蓋 `/health`、`initialize`、`tools/list`、`scan_source`、
    `preview_interpretation` 和 backend-ready `train` long-running boundary。
  - artifact 保存到 `artifacts/mcp/http-walkthrough.json` 和
    `artifacts/mcp/http-walkthrough.md`。
- artifact summary：
  - `health_ok`：`True`
...
- focused validation：
  ...
  - red first on old cancelled job changing to `cancel_requested`, then `1 passed`
...
MCP HTTP long-running job workflows、完整 import wizard UI 驗收或論文最終統計報告。
...
MCP HTTP long-running job workflow 完成。
...
click-through、mature import wizard label editor 或 MCP HTTP long-running job workflow 完成。
...
- observed result:
  ...
  - Human-like walkthrough status: `passed`; table geometry review checked `15` table/tree widgets
    with `0` findings.
  - Human-like walkthrough loaded Dataset table now runs after startup geometry settling at
    `1280x800`: `widget_width=1020`, `header_length=993`, `viewport_width=994`,
    `right_gap_to_boundary=0`, `horizontal_scrollbar_max=0`.
- commands:
  - `QT_QPA_PLATFORM=offscreen poetry run python scripts/dev/capture_human_like_product_walkthrough.py`
    -> exit `0`.
  - `QT_QPA_PLATFORM=offscreen poetry run python scripts/dev/capture_data_interpretation_replay.py`
...
真人 Windows click-through、interactive desktop 3D 或 MCP HTTP long-running job workflow。
