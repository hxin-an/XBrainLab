...
| `QT_QPA_PLATFORM=offscreen MNE_DONTWRITE_HOME=true poetry run pytest --capture=sys tests/integration/pipeline/test_e2e_training.py -q` | `9 passed` | Existing Training / Evaluation / Visualization panel regression coverage remains green after replacing direct training-option success truth and strengthening progress-table assertions. | Human desktop acceptance or complete zero-controller UI. | Continue splitting mock-heavy UI panel tests from product-smoke evidence. |
