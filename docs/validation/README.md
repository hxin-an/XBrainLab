...
| `poetry run pytest --capture=sys tests/unit/test_architecture_compliance.py -q` / `poetry run python tests/architecture_compliance.py` | `95 passed` / `Architecture compliant!` | Architecture guard now rejects generic non-`None` agent tool result assertions in `test_application_surface.py`, while allowing exact `ToolCommandResult` shape assertions. | Semantic proof for every LLM/tool test file. | Extend only to additional files after their weak assertions have stronger replacements. |
| `poetry run ruff check tests/architecture_compliance.py tests/unit/test_architecture_compliance.py tests/unit/llm/tools/test_application_surface.py` / `poetry run basedpyright tests/architecture_compliance.py tests/unit/test_architecture_compliance.py tests/unit/llm/tools/test_application_surface.py` | PASS / `0 errors, 0 warnings, 0 notes`. | Changed test and guard files are lint and type clean. | Runtime behavior by itself. | Pair lint/type checks with behavior tests. |
