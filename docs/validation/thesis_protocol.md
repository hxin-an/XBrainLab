...
刷新current claim。產品重建期間只使用`scripts/dev/run_stable_assistant_model_eval.py`的34-case
bilingual target gate；它要求exact stage、tool、parameters與schema，但不是thesis benchmark。
正式thesis runner必須等產品surface凍結後另以approved target cases重建，並重新定義repeat、resource
preflight、confidence interval與artifact schema；不得把產品34-case結果升格為thesis-ready rerun。
...
或thesis-candidate current evidence。Current產品層只有34-case bilingual target selection gate；
