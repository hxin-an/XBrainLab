...
| Assistant no-action precision | active | Frozen 50-case core保持歷史身分，另以24個雙語precision cases要求產品最終零誤動作；第一次raw generation、依現行target最多一次production format recovery與final Host outcome分開記錄，舊兩次預算artifact不追改。Final gate不得以format retry掩蓋semantic failure。Multi-action一律先詢問第一個action，本回合不部分執行。回覆自然度由同一SHA真人驗收，不以脆弱keyword gate冒充語意品質。 |
