...
| Assistant evaluator report schema target | approved, A2 pending | Current v11 仍存在到專屬 PR 合併為止；A2 target 將50-case core、24-case precision與7-case clarification分別放在`case_summaries`，`total`只表示81-case inventory完整性。Raw model、Host safety、direct admission、product outcome與overall pass只屬於獨立`candidate_gate`，不將不同分母混成一個accuracy，不保留模糊legacy `summary`。 |
