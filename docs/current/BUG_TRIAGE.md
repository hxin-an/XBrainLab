...
#### [BUG-ENV-003] 目前 `/mnt/d` Codex workspace 的預設 pytest capture 曾間歇性失敗
...
在目前 `/mnt/d/repos/XBrainLab` 的 Codex workspace 中，pytest 預設 capture mode 曾在 teardown 階段於 `_pytest/capture.py` 內拋出 `FileNotFoundError`，但最新 recheck 沒有穩定重現。
...
targeted slice 本身仍有 coverage，但這條問題目前需要重新驗證，因為最新 backend 與 IO recheck 沒有重現先前的 teardown crash。

### 備註

目前已將問題縮小到 `fd` capture，而不是 pytest collection 整體壞掉；但它已不再是最新一輪最清楚的 unattended blocker。
