...
class BasedpyrightRegressionError(RuntimeError):
    """Raised when exact-base type-check evidence cannot be produced."""
...
def _resolve_base(repo_root: Path, base_sha: str) -> str:
    git = shutil.which("git")
    if git is None:
        raise BasedpyrightRegressionError("git executable is unavailable")
    if not _FULL_SHA.fullmatch(base_sha):
        raise BasedpyrightRegressionError("base SHA must be a full 40-character SHA")
    resolved = _run(
    ...
    if resolved.returncode != 0 or exact.casefold() != base_sha.casefold():
        raise BasedpyrightRegressionError("authorized base SHA is unavailable")
    ancestor = _run(
    ...
    if ancestor.returncode != 0:
        raise BasedpyrightRegressionError(
            "authorized base SHA is not an ancestor of candidate HEAD"
        )
    return exact.casefold()


def _archive_base(repo_root: Path, base_sha: str, destination: Path) -> None:
    git = shutil.which("git")
    if git is None:
        raise BasedpyrightRegressionError("git executable is unavailable")
    try:
        ...
        raise BasedpyrightRegressionError("git archive timed out") from error
    if completed.returncode != 0:
        detail = completed.stderr.decode("utf-8", errors="replace").strip()
        raise BasedpyrightRegressionError(f"git archive failed: {detail}")
    ...
    with tarfile.open(fileobj=io.BytesIO(completed.stdout), mode="r:") as archive:
        for member in archive.getmembers():
            ...
            if relative.is_absolute() or ".." in relative.parts:
                raise BasedpyrightRegressionError("git archive contains unsafe path")
            target = destination.joinpath(*relative.parts)
            if not target.resolve().is_relative_to(destination_root):
                raise BasedpyrightRegressionError("git archive escapes destination")
            if member.isdir():
            ...
            if source is None:
                raise BasedpyrightRegressionError("git archive member is unreadable")
...
def _diagnostics(report_text: str, *, root: Path) -> Counter[Diagnostic]:
    try:
        ...
        raise BasedpyrightRegressionError("basedpyright output is not JSON") from error
    if not isinstance(report, dict):
        raise BasedpyrightRegressionError("basedpyright report is not an object")
    ...
    for raw in raw_diagnostics:
        if not isinstance(raw, Mapping):
            raise BasedpyrightRegressionError("basedpyright diagnostic is malformed")
        ...
        if (
            ...
            raise BasedpyrightRegressionError("basedpyright diagnostic is incomplete")
        try:
        ...
        normalized[
            (relative, severity, str(rule or ""), " ".join(message.split()))
        ] += 1
    return normalized
...
def run_regression_check(
    ...
    *,
    ...
    archive_base: ArchiveBase = _archive_base,
    analyze: Analyze = _analyze,
    resolve_base: ResolveBase = _resolve_base,
) -> tuple[int, dict[str, Any]]:
    """Analyze candidate and exact base with one executable and compare results."""
    root = repo_root.expanduser().resolve(strict=True)
    exact_base = resolve_base(root, base_sha)
    candidate_code, candidate_stdout, candidate_stderr = analyze(
        root,
    ...
    with tempfile.TemporaryDirectory(prefix="xbrainlab-basedpyright-base-") as raw_dir:
        ...
        archive_base(root, exact_base, base_root)
        base_code, base_stdout, base_stderr = analyze(
            base_root,
            basedpyright_executable,
        )
        if base_code not in {0, 1}:
            raise BasedpyrightRegressionError(
                "baseline basedpyright execution failed: " + base_stderr.strip()
            )
    ...
    result.update(
        {
            ...
            "baseline_return_code": base_code,
            "comparison_policy": (
                "relative-path,severity,rule,normalized-message multiset; "
                "source ranges excluded"
            ),
        }
    )
...
def main() -> int:
    ...
    except (BasedpyrightRegressionError, OSError) as error:
