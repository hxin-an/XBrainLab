...
- **Ablation Experiment Framework** (`scripts/agent/benchmarks/ablation_study.py`):
  ...
  - Per-condition evaluation with component contribution analysis (Δ from baseline)
  - Markdown + JSON report with failure type breakdown and case-level impact
  - CLI: `--model`, `--timeout`, `--delay`, `--dataset`, `--conditions`

- **Multi-Turn Conversation Benchmark**:
  - Test set: `scripts/agent/benchmarks/data/conversation_test_set.json` — 20 scenarios across 4 categories (coreference, context_memory, pipeline_sequence, correction)
  - Evaluator: `scripts/agent/benchmarks/multiturn_bench.py` — turn-by-turn evaluation with conversation history, state simulation, nudge-retry, per-category accuracy

- **RAG Hybrid Retrieval (BM25 + Semantic)**:
  - `XBrainLab/llm/rag/bm25.py`: Lightweight Okapi BM25 index (zero external deps)
  - `XBrainLab/llm/rag/retriever.py`: Hybrid scoring with configurable `hybrid_alpha` (default 0.7 — 70% semantic + 30% BM25)
  - Fetch 3× candidates for re-ranking, min-max normalize both scores, weighted interpolation

- **Generation Parameter Persistence** (ConfigManager expansion):
  - `LLMConfig.save_to_file()` / `load_from_file()` now persist `temperature`, `top_p`, `max_new_tokens`
  - `ModelSettingsDialog`: New "Generation Parameters" group with Temperature (0–2), Top-p (0–1), Max Tokens (64–8192) spinboxes

- **Backend Headless Verification** (`scripts/agent/benchmarks/validate_headless.py`):
  - 26 import tests + 5 instantiation tests + Qt side-effect check
  - Validates backend/LLM core can run without Qt/display server (Docker/CI prerequisite)

### Changed
- Bumped version to 0.5.6.
- `RAGRetriever.__init__` now accepts optional `hybrid_alpha` parameter.
- `RAGRetriever.initialize()` builds BM25 index alongside Qdrant collection.
