...
supports: current visual state for assistant One step/Guided workflow narrow surfaces, model selection, data splitting, and evaluation metrics table polish
...
```bash
QT_QPA_PLATFORM=offscreen poetry run python scripts/dev/capture_ui_polish_surfaces.py --only assistant-ask-narrow.png --only assistant-workflow-narrow.png --only assistant-loading.png --only assistant-failed.png --only assistant-recovery-loading.png
```
...
- `assistant-workflow-narrow.png` (420 x 650, ready Guided workflow processing)
