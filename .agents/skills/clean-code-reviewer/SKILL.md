...
- 讓 `ApplicationService`、controller、UI panel 或 agent controller 承擔更多不相干責任。
- 重新加入已物理移除的 `BackendFacade`，或新增等價 generic facade 形成第二個 command spine。
- 把 temporary compatibility path 變成永久 product path。
- 新增 fallback 讓 real product runtime 默默繞過 command / policy / validation。
- 讓 UI、agent、headless scripts 各自判斷 workflow readiness、label/event meaning 或 state truth。
...
- `BackendFacade` 是否保持物理移除，且沒有用另一個 generic wrapper 重新建立第二個 command
  spine？
...
- product UI / agent / headless scripts 是否以 Data Interpretation flow 為主？
...
- UI enablement、agent tool exposure 與 headless command admission 是否讀同一套 command
  capability？
...
- UI / agent / headless 主流程若受影響，要跑對應 walkthrough、command smoke 或 tool-call
  regression。
- MCP validation 只在使用者明確要求 MCP work 時加入；它不是一般 product handoff gate。
...
- UI / agent / headless 有任一入口繞過 `ApplicationService` 或 capability policy。
- `BackendFacade` 或等價第二 command spine 被重新加入 product runtime。
...
- 新增 Data Interpretation / backend command / agent tool 時：檢查是否放在正確邊界。
- 使用者明確要求 MCP work 時：另外套用 `mcp-adapter-reviewer`，不要把該 scope 擴張成
